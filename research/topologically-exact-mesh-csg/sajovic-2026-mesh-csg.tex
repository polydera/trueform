...
\textbf{Topology survives the float boundary.} An arrangement or boolean is never the whole pipeline. Mesh CSG is a step in something larger --- a model on its way to finite-element analysis, mesh editing, or export. Several recent systems are exact within their own representation: EMBER's bounded-width plane kernel~\cite{Trettner2022} guarantees iterated CSG at interactive rates without leaving its representation; L\'evy's pipeline~\cite{Levy2025} carries arithmetic expansions and a multiprecision fallback to keep the same guarantee. These are remarkable results within their exact universe. But use-cases rarely stay within that universe for more than one operation. The moment exact coordinates materialise back to IEEE double, new intersections may appear that were not in the exact result: EMBER's snap-rounding step at the float boundary makes this concrete~\cite{Trettner2022}, and L\'evy reports the same phenomenon~\cite{Levy2025}. The boundary works in reverse too: inputs come from upstream tools that have already crossed it, so a CSG operation never receives truly clean input either. Pushing precision arbitrarily high inside the system does not change this --- uncertainty re-enters at the boundary regardless of how it was held inside. The pragmatic response, taken here, is to absorb it within the pipeline, using the structure topology imposes. The only invariant that survives this boundary, in either direction, is topology.
...
The pipeline of Section~\ref{sec:graphs} groups non-manifold edges into equivalence classes by canonical permutation. Each class votes on its canonical permutation and emits fragment-side merge pairs; cross-bundle nesting (below) emits additional domain-id merge pairs that close the case of geometrically disjoint bundles. A dense equivalence-class map consumes the combined merge buffer, assigning one \textbf{domain id} per fragment-side. The final per-face per-side labels follow by lookup in one parallel pass: $\text{labels}[f][s] = \text{domain\_of}[2c + s]$ for face $f$ in MEL component $c$. The whole pipeline operates on the reduced graph (buffers proportional to the number of MEL components), not on per-face data.
