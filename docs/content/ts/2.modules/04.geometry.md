...
// Double-precision mesh — pass { dtype: "float64" } as a trailing option
const sphere64 = tf.sphereMesh(1.0, 20, 20, { dtype: "float64" });
```

All generators accept a trailing `{ dtype: "float32" | "float64" }` option (default: `"float32"`). For `tubeMesh`, the dtype is inferred from the input curves.

| **Function** | **Parameters** | **Description** |
|---|---|---|
| `sphereMesh` | `radius`, `stacks`, `segments`, `[opts]` | UV sphere |
| `cylinderMesh` | `radius`, `height`, `segments`, `[opts]` | Cylinder along z-axis |
| `boxMesh` | `width`, `height`, `depth`, `[ticks]`, `[opts]` | Axis-aligned box |
| `planeMesh` | `width`, `height`, `[ticks]`, `[opts]` | Flat plane in XY |
| `tubeMesh` | `curves`, `radius`, `[radialSegments]` | Tube mesh from curves (dtype inherits from curves) |
...
Measure every face of a mesh — one array per measure, each of shape `[F]` in the mesh's dtype.
...
|---|---|---|
| `quality` | `NDArrayFloat32 \| NDArrayFloat64 [F]` | The triangle measure: `1` for equilateral, approaching `0` for a sliver; a face that is not a triangle reads `-1` |
| `minAngle` | `NDArrayFloat32 \| NDArrayFloat64 [F]` | Smallest corner angle, in radians |
| `maxAngle` | `NDArrayFloat32 \| NDArrayFloat64 [F]` | Largest corner angle, in radians |
| `aspectRatio` | `NDArrayFloat32 \| NDArrayFloat64 [F]` | Longest side over the shortest, infinite where a side has no length |

The corner angles and the aspect ratio are stated for a face of any arity. An angle is unsigned and lies in `[0, pi]`, so a reflex corner of a non-convex face reads its explement.
...
```ts
const { edges, angles } = tf.dihedralAngles(mesh);
// edges  : NDArrayInt32 [N, 2] — vertex index pairs, one per undirected edge
// angles : NDArrayFloat32 | NDArrayFloat64 [N] — radians, aligned with edges
```

One entry per undirected edge, reached through the manifold edge link's representative. The angle is between the face normals, so a flat surface reads `0`. A boundary or non-manifold edge joins no pair of faces and turns through no angle, so it is not stated. The `manifoldEdgeLink` and `normals` the call reads are cached on the mesh.
...
**A face whose loop crosses itself is resolved, not dropped:** it states its crossing and mints the identity that names it. The returned mesh then owns its own point table; a mesh no face of which needs resolving mints nothing and shares the input's points, which is the mesh in practice.
::
