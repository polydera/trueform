...
A mesh is manifold when the faces around every vertex are one fan, which also says every edge carries at most two of them.

```ts
tf.isManifold(mesh);     // true if every vertex's faces are one fan
tf.isNonManifold(mesh);  // true if any edge has 3+ faces, or any vertex splits into fans
```

Non-manifold edges typically occur at T-junctions where three or more faces meet at a single edge; non-manifold vertices where two fans touch at a single point. Face winding does not enter the verdict.

::tip{icon="i-lucide-info"}
Use `tf.nonManifoldEdges(mesh)` or `tf.nonManifoldVertices(mesh)` to get the offending features when `isNonManifold` returns `true`.
::
...
// vertices : OffsetBlockedBuffer — block i is rim i walked
// faces    : OffsetBlockedBuffer — block i is the face across each of its edges
// closed   : NDArrayBool [R]     — whether rim i's last edge runs back to its first vertex

for (let i = 0; i < vertices.length; i++) {
  const rim = vertices.get(i);        // NDArrayInt32 — vertex ids in walk order
  const carriers = faces.get(i);      // NDArrayInt32 — one face per rim edge
  const isClosed = closed.data[i] !== 0;
}
```
...
Find the vertices whose faces are not one fan — an edge at the vertex carries 3+ faces, or its faces fall into several pieces meeting at the vertex alone.

```ts
const vertices = tf.nonManifoldVertices(mesh);  // NDArrayInt32 [N], ascending
```

The walk is on the mesh's `faceMembership`, so no half-edge structure is built. Face winding does not enter the verdict, and a vertex no face names is not reported.
...
Give every fan at a vertex a vertex of its own. The fan holding the vertex's smallest face keeps its id; every other fan takes a fresh id carrying the same coordinates, and that fan's corners are rewired onto it.

```ts
const { mesh: split, pointMap } = tf.splitNonManifoldVertices(mesh);
// split    : Mesh — same dtype as the input
// pointMap : NDArrayInt32 [P] — for each output point the input point it
//            copies, an original itself

tf.isManifold(split);  // true when no edge of the input carried 3+ faces
```

Faces keep their ids, their arity and their winding, and the points are the input's followed by the minted copies.

An edge 3+ faces carry is crossed by no fan, and separating the fans it holds apart would tear that edge into boundary copies. That is another verb's work: a vertex any of whose edges carries 3+ faces is left exactly as it was, and `tf.nonManifoldVertices` still names it. This one separates only the fans that come apart without cutting an edge.
...
### Demanding the constraints verbatim

By default crossings are resolved by splitting: an output point is created where two constraints meet. Pass `splitConstraints: false` to demand the constraints exactly as given — if any two of them cross, the result is **empty**. That emptiness is the answer to "do these constraints cross", not an error.

```ts
// Rectangle outline + 2 diagonals crossing at the centre
const split = tf.cdt(points, { edges });                            // crossing split, centre point added
const verbatim = tf.cdt(points, { edges, splitConstraints: false }); // verbatim.faces.shape[0] === 0
```

Without crossings the two agree. The option is ignored when no `edges` are given.
...
const rims = await tf.async.boundaryRims(mesh);
const { labels, nComponents } = await tf.async.connectedComponents(mesh, "edge");
const { mesh: split } = await tf.async.splitNonManifoldVertices(mesh);
