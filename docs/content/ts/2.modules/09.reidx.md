...
`tf.splitIntoDomains` is the per-side counterpart of `tf.splitIntoComponents`, keyed by the `DomainLabelsResult` returned by `tf.domainLabels` (see [Topology module](/ts/modules/topology#volumetric-domains)). Each face contributes up to two emissions (one per side); sentinel-bound sides are dropped, and side-0 emissions reverse winding so every output submesh has outward-of-domain normals.

```ts
const dl = tf.domainLabels(mesh, { ignoreOpenFragments: true });
const { components, labels } = tf.splitIntoDomains(mesh, dl);
// components : Mesh[]         — one watertight submesh per domain
// labels     : NDArrayInt32   — the domain id for each emitted submesh
```

The second argument accepts any object with `{ labels, nDomains }` — the full `DomainLabelsResult` (with `outerShellLabel`) is assignable; `outerShellLabel` is accepted but not consumed.

Async: `await tf.async.splitIntoDomains(mesh, dl)`.
...
| `splitIntoDomains` | Per-face per-side `DomainLabelsResult` | Single mesh |
...
const dl = await tf.async.domainLabels(mesh);
const submeshes = await tf.async.splitIntoDomains(mesh, dl);
```
