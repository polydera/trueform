...
Read an STL file (binary or ASCII) from a buffer. Only vertex positions and face connectivity are read — per-face normals stored in the STL are ignored (use `mesh.normals` to compute them). STL coordinates are float32 by format spec, so the returned mesh is always `dtype: "float32"`.
...
Pass `{ dtype: "float64" }` to read coordinates at double precision (default: `"float32"`). The same option applies to `readObjData`:

```ts
const mesh = tf.readObj(bytes, { dtype: "float64" });
const data = tf.readObjData(bytes, { dtype: "float64" });
```
...
Writes binary STL. Per-face normals are included if available, otherwise zero normals are written. STL output is float32 by format spec — float64 mesh coordinates are downcast to float32 at write time.
...
Writes ASCII OBJ with vertex positions and face connectivity only (no normals, UVs, or materials). Output precision matches the mesh dtype: float32 emits `%.9g`, float64 emits `%.17g` (exact round-trip for IEEE 754 doubles).
