...
Default: `mode: "primitives"`, `tolerance: 0` (exact), `triangulation: "cdt"` (`"refinedCdt"` quality-refines the cut surface). A one-mesh entry implies `within`, so the mesh's own self records and its self-crossing contours are both in.
