...
- **`f`** — `NDArrayInt32` forward mapping of length N (original count). `f[oldId]` gives the new index for kept elements (`< f.length`). The sentinel value `f[oldId] === f.length` marks removed elements (degenerate, dropped duplicate, or unreferenced).
