...
`trueform` is a TypeScript library for real-time geometric processing. WASM-backed NDArrays with support for vectorized numerical operations and spatial queries. Arrangements, booleans, registration, remeshing — at native speed in the browser and Node.js. Exact and robust to non-manifold flaps and pipeline artifacts. NDArrays in, NDArrays out.
