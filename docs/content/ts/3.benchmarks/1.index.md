...
The TypeScript SDK uses the same C++ core compiled to WebAssembly, so performance characteristics are identical (accounting for WASM overhead). See the [C++ benchmarks](/cpp/benchmarks) for experimental setup, detailed charts, and scaling analysis.
...
To learn more about exact predicates and canonical topology, see [Intersect](/ts/modules/intersect#intersection-modes), [Cut](/ts/modules/cut#supported-input), and [Research](/cpp/about/research).
::
