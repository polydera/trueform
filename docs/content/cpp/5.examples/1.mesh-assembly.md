...
A complete workflow: creating geometry, understanding topology, assembling watertight meshes, and combining them correctly.

**Source:** [mesh_assembly.cpp](https://github.com/xlabmedical/trueform/tree/main/examples/src/mesh_assembly.cpp)
...
With `tf::dynamic_size`, faces are stored in a `tf::offset_block_buffer<int, int>`. Point indices are automatically offset during concatenation.
...
auto [center_id, _] = tf::neighbor_search(
    tf::make_form(sheet_tree, cone_sheet.points()),
