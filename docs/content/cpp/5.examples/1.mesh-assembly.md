...
`release()` transfers ownership of the underlying array to you. It was allocated by trueform's allocator — free it with `tf::deallocate<T>(ptr)` (the buffer's element type `T`), not `delete[]`.
...
```cpp
auto [pencil, labels_, fl_] = tf::make_boolean(
...
auto [bicylinder, labels_, fl_] = tf::make_boolean(
...
See [Topology](/cpp/modules/topology), [Geometry](/cpp/modules/geometry), and [Arrangement](/cpp/modules/arrangement) for details.
::
