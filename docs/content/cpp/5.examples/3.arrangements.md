...
Declaring the plane a [sheet](/cpp/modules/csg#sheets) makes `op(3)` an oriented separator: its operand bit means "behind the sheet's normal" (−Z here), so the knife cuts volumes through the same boolean algebra without enclosing one. Every query below reuses this build.
