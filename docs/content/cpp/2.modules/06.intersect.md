...
Default mode: `sos`. With two meshes every contour is of the one class (A,B); crossings among them — e.g. from disjoint components of one mesh — are resolved by `resolve_self_crossing_contours` or `within`.
