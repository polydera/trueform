...
Contour crossing resolution is configured per function with sensible defaults. For detecting where a **single mesh intersects itself**, use [`tf::make_self_intersection_curves`](#self-intersection-curves); for whether it does at all, [`tf::has_self_intersections`](#self-intersection-test).
::
...
tf::intersect_config{tf::intersect_mode::primitives |
                     tf::intersect_mode::within, 1e-6}
```

#### Mode

`tf::intersect_mode` carries the classifier and optional `within` request in one bitmask. The canonical spelling is `primitives | within`.

| Mode | Description |
|------|-------------|
| `tf::intersect_mode::primitives` | Full 5-type classification (VV, VE, EE, VF, EF). Handles shared edges, shared vertices, and coplanar faces. The default on every entry. |
| `tf::intersect_mode::sos` | SoS (Simulation of Simplicity) perturbation. Every contact is perturbed into a generic crossing, so all records are edge-face and none is ever coplanar. On an arrangement that means coplanar walls do not pool and the domains they would have separated stay joined — the caller's choice. |

#### Within

`within` asks for each form's own self-intersections. Its two halves — the form's self records, and the resolution of crossings within one contour class — are one request and never separable. A one-form build implies it.

Spell it whenever a form can self-overlap, e.g. meshes concatenated into one operand; see [CSG: Self-Arrangements](/cpp/modules/csg#self-arrangements).

#### Contour crossings

Crossings between contours are resolved unconditionally, not on request: between contour classes whenever a third tag exists to make such a pair, and within one class whenever the run is `within`. That was always the behaviour of every default; there is no flag to turn it off with.

```cpp
// The N-form default already resolves what its arity can make
...
`tf::has_self_intersections` is the verdict of `tf::polygon_intersections`' one-form build: true exactly when that build would state a self record.

```cpp [has_self_intersections.cpp]
bool meets_itself = tf::has_self_intersections(polygons);

// A form already carrying the structures pays for none of them
auto form = polygons | tf::tag(tree) | tf::tag(fm) | tf::tag(mel);
bool tagged_answer = tf::has_self_intersections(form);

// The lattice is resolved from the input, or stated as everywhere else
bool at_int64 = tf::has_self_intersections<tf::exact::int64>(polygons);
```

This is that build's discovery tier, elections and all, stopped at the first record — so a mesh that meets itself is answered without the rest of it being seen, and a mesh that does not is the whole discovery and nothing above it. A shared vertex or edge is not itself a contact; any other touching — between neighbouring faces too — is one, and the lattice decides it exactly.

Missing structures — the tree, the face membership, the manifold edge link — are built for the call, so a bare mesh is a valid operand. The input must be 3D.

::tip{icon="i-lucide-info"}
For where the mesh meets itself rather than whether, use [`tf::make_self_intersection_curves`](#self-intersection-curves).
::
...
Default config: `primitives`, exact, not `within`. With two meshes every contour is of the one class (A,B); crossings among them — e.g. from disjoint components of one mesh — are resolved when the run is `within`.
...
Default config: `primitives`, exact. With 3+ meshes, contours from different mesh pairs can cross on a shared face; the arity makes such a pair possible, so those crossings are resolved.
...
Default config: `primitives`, exact. A one-form build implies `within`, so the mesh's self records are emitted and crossings within one contour class are resolved — different face pairs of the same mesh can produce them.
...
Arity and `tf::intersect_config` determine what is computed. There is one class and no "between" / "within" aliases: a one-form build is the self arrangement (`within` is implied), a two-form or N-form build always emits cross records and adds per-form self records when the config asks for `within`.

Every build takes a `tf::exact::input_lattice` — where each original vertex of each operand stands on the integer lattice, built once over the union of the operands:

```cpp [low_level_polygon.cpp]
tf::exact::input_lattice<int, float, tf::exact::int32> lattice;
lattice.build(
    tf::exact::make_vertex_converter<tf::exact::int32, float>(form1, form2),
    [&](int tag, auto &&f) {
      if (tag == 0)
        f(form1);
      else
...
tf::polygon_intersections<int, float> ibp;   // lattice int per the real type

ibp.build(form1, form2, lattice, tf::intersect_mode::primitives);  // cross records
ibp.build(form1, form2, lattice,                                   // + self records
          tf::intersect_mode::primitives | tf::intersect_mode::within);
```

The range build and the one-form build take the same shape — a lattice over their own operands, then the forms:

```cpp
ibp.build(tf::make_range(forms, forms + 3), lattice_n,   // every pair
          tf::intersect_mode::primitives);
ibp.build(form1, lattice_1, tf::intersect_mode::primitives);  // self only
```
