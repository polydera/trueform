...
The `tf::make_k_ring` function extends the 1-ring concept to compute all vertices reachable within k hops along mesh edges.
...
auto k_ring_2 = tf::make_k_ring(v_link, 2);
...
auto k_ring_5 = tf::make_k_ring(v_link, 5);

// Include the seed vertex itself (inclusive mode)
auto k_ring_inclusive = tf::make_k_ring(v_link, 2, true);
```
