...
| `distance`   | Distance |
| `intersects` | `bool` |
| `signed_distance` | Distance to a mesh, negative inside |
...
`tf::signed_distance(polygons, point)` signs the distance by the Baerentzen–Aanes pseudonormal of the closest feature, so the mesh must carry a tree, a `face_membership` and a `manifold_edge_link`:

```cpp [signed_distance.cpp]
auto form = polygons | tf::tag(tree) | tf::tag(membership) | tf::tag(mel);
double sd = tf::signed_distance(form, point);   // < 0 inside
```

### Winding Numbers

`tf::winding_number(polygons, point)` is the generalized winding number — approximately 1 inside a closed surface, 0 outside, and a graceful fractional value for open sheets and soups. The tree carries a moment sidecar built once by `tf::make_winding_moments` — the expansion to Taylor order two: dipole, second moment, and the quadrupole tier; far nodes answer by the folded expansion, near leaves by exact solid angles. The batch overload writes a range of query points in one parallel pass, and `winding_config{beta}` trades accuracy against traversal depth (default 2).

```cpp [winding_number.cpp]
auto moments = tf::make_winding_moments(tree, polygons);
auto form = polygons | tf::tag(tree) | tf::tag(moments);
double w = tf::winding_number(form, point);       // ~1 inside, ~0 outside
tf::winding_number(form, query_points, out);      // batch
```

A form tagged with winding moments signs `tf::signed_distance` by the winding number instead of the pseudonormal — the graceful sign for open sheets and soups — and then needs only the tree:

```cpp [signed_distance_winding.cpp]
auto wform = polygons | tf::tag(tree) | tf::tag(moments);
double sd = tf::signed_distance(wform, point);    // sign = winding > 0.5
```
