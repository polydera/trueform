...
// Returns std::pair<std::vector<tf::polygons_buffer<...>>, tf::buffer<label_type>>
...
// Returns std::pair<std::vector<tf::segments_buffer<...>>, tf::buffer<label_type>>
```
...
`tf::split_into_domains` is the per-side counterpart of `tf::split_into_components`, keyed by `tf::domain_labels`. Each face contributes up to two emissions, one per side; sentinel-bound sides are dropped, and side-0 emissions reverse the stored winding so every output buffer has outward-of-domain normals.

```cpp [split_into_domains.cpp]
auto labels = tf::make_domain_labels(
    polygons, tf::domain_config::ignore_open_fragments);

auto [submeshes, domain_ids] =
    tf::split_into_domains(polygons, labels);
// Returns std::pair<std::vector<tf::polygons_buffer<...>>, tf::buffer<label_type>>

for (auto [submesh, domain_id] : tf::zip(submeshes, domain_ids)) {
    // submesh is a watertight surface of the domain, outward-oriented
    auto vol = tf::signed_volume(submesh.polygons());
}
```

::tip{icon="i-lucide-info"}
**Relationship with `split_into_components`**:
- `tf::split_into_components`: per-face label → one submesh per component.
