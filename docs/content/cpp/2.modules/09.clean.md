...
`tf::cleaned` (on polygons and segments) also accepts a `tf::clean_config_t<Tolerance>` for fine-grained control over which optional passes run. Vertex deduplication and degenerate-primitive removal **always** run; the flags only gate the two optional passes:

```cpp [clean_config.hpp]
template <typename Tolerance> struct clean_config_t {
  Tolerance tolerance{};
  bool remove_duplicate_primitives = true;  // dedup faces / edges
  bool remove_unreferenced_points  = true;  // drop unused vertices
};
```

Build one with the `tf::clean_config(...)` factory and pass it to `cleaned`:

```cpp [clean_with_config.cpp]
// Keep duplicate faces but still merge vertices and drop unreferenced ones.
auto cfg = tf::clean_config(1e-6f, /*remove_dup=*/false, /*remove_unref=*/true);
auto out = tf::cleaned(polygons, cfg);

// With index maps.
auto [out2, face_im, point_im] =
    tf::cleaned(polygons, cfg, tf::return_index_map);
```

::note{icon="i-lucide-info"}
The config overload applies to polygons and segments. Points, curves, and soups don't accept a config — neither the dedup flag nor the unref-prune flag has a meaning for those primitives.
::
...
An index map's forward array `im.f()` has one entry per **input** element. The convention for marking removed inputs is uniform across all of trueform: a removed input is set to `im.f().size()` (the input count).

```cpp [index_map_sentinel.cpp]
if (im.f()[i] == im.f().size()) {
  // Input i was removed: degenerate, dropped duplicate, or unreferenced.
} else {
  auto new_id = im.f()[i];
  // Valid output index. Multiple inputs can share the same new_id when
  // they were merged (e.g. duplicate vertices folded into a representative).
}
```

Survivors -- including duplicates folded into a representative -- always satisfy `im.f()[i] < im.f().size()`. Only fully-removed inputs hit the sentinel.

::tip{icon="i-lucide-shuffle"}
See [Core](/cpp/modules/core#index-maps) for index map details, and [Reindex](/cpp/modules/reindex) for applying them to geometric data.
::
...
// With clean_config (polygons / segments only)
auto cfg = tf::clean_config(tolerance, /*remove_dup=*/false, /*remove_unref=*/true);
auto [face_map, point_map] = tf::make_clean_index_map(polygons, cfg);
