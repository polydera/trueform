...
### Transforming and Translating Forms

`tf::polygons`, `tf::segments`, and `tf::points` views support bulk affine transforms in place (`tf::transform`, `tf::translate`) or as a copy (`tf::transformed`, `tf::translated`).

```cpp [transforming_forms.cpp]
auto mesh = tf::make_sphere_mesh<int>(1.0f, 32, 32);
auto t = tf::make_rotation(tf::deg<float>(45), tf::axis<2>);

// In-place: mutate the underlying points buffer through the view.
tf::translate(mesh.polygons(), tf::make_vector(1.f, 0.f, 0.f));
tf::transform(mesh.polygons(), t);

// Copy: returns a new polygons_buffer; original untouched.
auto rotated = tf::transformed(mesh.polygons(), t);
auto shifted = tf::translated(mesh.polygons(), tf::make_vector(0.f, 0.f, 5.f));
```

`translate` walks points with `p += v`; `transform` runs `p = tf::transformed(p, t)`. Both `transform` and `transformed` accept either a `transformation_like` or a `frame_like`.

::note{icon="i-lucide-info"}
In-place overloads `static_assert` that the view does not carry a frame policy. Tagged frames are composed at read time, so mutating the underlying points would silently double-apply the transform.
::

::tip{icon="i-lucide-zap"}
Most trueform algorithms read points through `tf::frame_of`, so they consume tagged inputs directly. If you just need a transformed mesh as input to another algorithm, prefer the lazy form via [Tagging Transformations and Frames](#tagging-transformations-and-frames).
::
...
Compose multiple ranges for lockstep iteration. Each element is a tuple of corresponding elements from all input ranges. Length equals the shortest range; use a parallel algorithm over the zipped range when execution should be parallel:

```cpp [zip.cpp]
auto points = tf::make_points<3>(coords);
auto normals = tf::make_unit_vectors<3>(normal_data);

for (auto [pt, normal] : tf::zip(points, normals)) {
    // pt is point_view<float, 3>, normal is unit_vector_view<float, 3>
    auto offset = pt + normal * 0.1f;
}

// Works with any number of ranges
for (auto [id, pt, normal] : tf::zip(tf::make_sequence_range(n), points, normals)) {
    // id is the index, pt and normal are views
}
```

#### `tf::zip_with<Tag>`

Zip ranges and construct a typed struct per element. Define a struct whose fields match the zipped element types in order—`zip_with` constructs it automatically via `tf::apply<Tag>`:

```cpp [zip_with.cpp]
struct vertex_data {
    tf::point_view<float, 3> position;
    tf::unit_vector_view<float, 3> normal;
};

auto vertices = tf::zip_with<vertex_data>(points, normals);

for (auto v : vertices) {
    v.position;  // named access, no get<0>
    v.normal;    // named access, no get<1>
    auto offset = v.position + v.normal * 0.1f;
}
```

This is the AOS (array-of-structs) iteration pattern—data lives in separate arrays (SOA), but you iterate as if it were a single struct per element.

::tip{icon="i-lucide-info"}
`zip_with` constructs `Tag` from the zipped element types via `tf::apply<Tag>`. For simple cases, aggregate initialization works directly. When the zipped types need unpacking, provide a constructor.
::
...
See [Policies](#policies) to learn how the policy mechanism enables expressive and performant interaction between [Spatial](/cpp/modules/spatial), [Geometry](/cpp/modules/geometry), [Topology](/cpp/modules/topology), [Intersect](/cpp/modules/intersect), and [Arrangement](/cpp/modules/arrangement) modules.
...
::tip{icon="i-lucide-info"}
Area functions respect [tagged transformations](#tagging-transformations-and-frames). When a range has a tagged frame, areas are computed in the transformed coordinate space.
::
...
::tip{icon="i-lucide-info"}
All edge length functions respect [tagged transformations](#tagging-transformations-and-frames). When a range has a tagged frame, lengths are measured in the transformed coordinate space. When untagged, the identity frame is used at zero cost.
::
...
using poly_coord_t = tf::coordinate_type<decltype(polygons)>;  // float
using point_coord_t = tf::coordinate_type<decltype(point)>;    // float
...
using common_t = tf::coordinate_type<Policy0, Policy1>;
...
coords.reserve(1000);   // Reserve capacity for appended values
coords.push_back(1.0f);
coords.emplace_back(2.0f);

// Or allocate a final-sized uninitialized output and fill its elements
tf::buffer<float> output;
...
// When finished, free it with the matching typed deallocate:
tf::deallocate<float>(raw_ptr);
...
- Allows you to take ownership of the underlying array. It was allocated by trueform's allocator — free it with `tf::deallocate<T>(ptr)` (the buffer's element type `T`, e.g. `tf::deallocate<float>`), not `delete[]`
...
See the [Reindex](/cpp/modules/reindex) module for applying index maps and filtering, [Clean](/cpp/modules/clean) for deduplication, and [Remesh](/cpp/modules/remesh) for decimation and edge collapse. All can return index maps via `tf::return_index_map`.
::
...
### Cancellation-isolated iteration

The `tf::isolated` overload of `tf::parallel_for_each` visits every element
through an isolated TBB task-group context. Use it only when an operation must
complete even if its caller's task group has already been cancelled:

```cpp
#include <trueform/core.hpp>

tf::parallel_for_each(values, [](auto &value) { value.update(); }, tf::isolated);
```

The range must remain alive for the call. The callback can execute concurrently
and must safely handle different elements in parallel.
...
`tf::generic_generate` provides parallel generation of variable-length data with automatic block-local state management:
...
// Generate with block-local state to avoid repeated allocations
tf::buffer<int> result_data;
tf::generic_generate(input_range, result_data,
    tf::small_vector<int, 10>{},  // Block-local work buffer
