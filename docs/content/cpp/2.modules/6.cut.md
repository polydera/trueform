...
See [Research Foundation](/cpp/getting-started#research-foundation) for the conceptual model and [Publications](/cpp/about/publications) for formal definitions.
::
...
auto tagged = mesh.polygons() | tf::tag(fm) | tf::tag(mel);

auto form1 = tf::make_form(frame, tree, tagged);
auto form2 = tf::make_form(tree, tagged);
