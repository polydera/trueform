...
To also compute principal directions, pass two additional `unit_vectors_buffer` arguments:

```cpp [compute_principal_curvatures_with_directions.cpp]
tf::buffer<float> k0, k1;
k0.allocate(n_vertices);
k1.allocate(n_vertices);
...
tf::compute_principal_curvatures(tagged, k0, k1);
```

The k-ring size defaults to 2. For smoother estimates, increase k:

```cpp
tf::compute_principal_curvatures(polygons, k0, k1, /*k=*/3);
tf::compute_principal_curvatures(polygons, k0, k1, d0, d1, /*k=*/3);
```
