...
Compute the two principal curvatures (k1, k2) at each vertex:

```cpp [compute_principal_curvatures.cpp]
tf::buffer<std::array<float, 2>> curvatures;
curvatures.allocate(polygons.points().size());

tf::compute_principal_curvatures(polygons, curvatures);

for (auto [k1, k2] : curvatures) {  // k1 >= k2
    float gaussian = k1 * k2;       // Gaussian curvature
    float mean = (k1 + k2) / 2;     // Mean curvature
}
```

The function uses the policy system. You can tag precomputed structures.

```cpp
auto tagged = tf::make_polygons(faces, points | tf::tag_normals(normals))
    | tf::tag(vertex_link);

tf::compute_principal_curvatures(tagged, curvatures);
```

The k-ring size defaults to 2. For smoother estimates, increase k:

```cpp
tf::compute_principal_curvatures(polygons, curvatures, /*k=*/3);
```
...
tf::compute_shape_index(tagged, curvatures);
```

The k-ring size defaults to 2. For smoother estimates, increase k:

```cpp
tf::compute_shape_index(polygons, curvatures, /*k=*/3);
```
