...
|--------|------------|---------|
| `point_tag_labels` | output point | input mesh tag (`0..N-1`) |
| `point_labels` | output point | input point id within its mesh |
| `face_tag_labels` | output face | input mesh tag |
| `face_labels` | output face | input face id within its mesh |
| `point_f` | `[tag][input point id]` | output point index (forward) |
| `n_original_points` | — | outputs `≥` this are created points |
| `n_original_faces` | — | boundary between original and cut faces |
| `n_output_points` | — | total output points; the `end` sentinel value |
...
**Created points and the `end` sentinel.** Output points split into kept originals `[0, n_original_points)` and created intersection points `[n_original_points, n_output_points)`. A created point has no input origin, so its inverse labels carry the sentinel value `n_output_points` — one past every real output index, the same idiom [`tf::index_map`](/cpp/modules/core) uses for unmapped entries:

```cpp
bool is_created = o >= imap.n_original_points;
// equivalently (N-mesh):  imap.point_tag_labels[o] == imap.n_output_points
```

The forward `point_f` emits the same sentinel for any input point with no output. Faces never carry the sentinel — a cut face is a piece of an input face, so it always keeps a real origin.
