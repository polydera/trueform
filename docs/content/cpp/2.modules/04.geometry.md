...
```cpp [make_tube_mesh_single.cpp]
auto tube = tf::make_tube_mesh(curve, 0.01f, 8);  // curve, radius, radial_segments
// Returns tf::polygons_buffer<int, float, 3, 3>
```

**Multiple curves:**

```cpp [make_tube_mesh_multi.cpp]
// From intersection curves
auto curves_buf = tf::make_intersection_curves(form0, form1);
auto tubes = tf::make_tube_mesh(curves_buf.curves(), 0.01f, 12);

// From boundary paths
auto boundary = tf::make_boundary_paths(polygons);
auto boundary_curves = tf::make_curves(boundary, polygons.points());
auto tubes = tf::make_tube_mesh(boundary_curves, 0.005f, 8);
```
...
Extract edges where the dihedral angle between adjacent faces exceeds a threshold:

```cpp [make_sharp_edges.cpp]
// Edges sharper than 30 degrees — blocked_buffer<Index, 2>
auto edges = tf::make_sharp_edges(polygons, tf::deg(30.f));

// With pre-computed manifold edge link and normals
auto tagged = polygons | tf::tag(mel) | tf::tag_normals(normals.unit_vectors());
auto edges = tf::make_sharp_edges(tagged, tf::deg(30.f));

// Visualize as segments
auto segments = tf::make_segments(edges, polygons.points());
```

Boundary and non-manifold edges are excluded — see [Topology](/cpp/modules/topology) for [`make_boundary_edges`](/cpp/modules/topology#boundary-detection) and [`make_non_manifold_edges`](/cpp/modules/topology#non-manifold-edge-detection). If manifold edge link or normals are not tagged, they are computed internally.

For the angle itself at every edge two faces share, rather than the edges past a threshold, see [Dihedral Angles](#dihedral-angles).
...
`tf::compute_face_quality` measures every face, one flat buffer per measure, each indexed by face:

```cpp [compute_face_quality.cpp]
auto measured = tf::compute_face_quality(polygons);

for (auto [q, a] : tf::zip(measured.quality, measured.aspect_ratio)) {
    // q: the triangle measure, a: longest side over shortest
}
```

Or destructure the four buffers directly:

```cpp
auto [quality, min_angle, max_angle, aspect_ratio] =
    tf::compute_face_quality(polygons);
```

| Measure | Type | Description |
|---------|------|-------------|
| `quality` | `tf::buffer<T>` | The triangle measure: `1` for equilateral, approaching `0` for a sliver; a face that is not a triangle has none and reads `-1` |
| `min_angle` | `tf::buffer<tf::rad<T>>` | Smallest corner angle |
| `max_angle` | `tf::buffer<tf::rad<T>>` | Largest corner angle |
| `aspect_ratio` | `tf::buffer<T>` | Longest side over the shortest, infinite where a side has no length |

The corner angles and the aspect ratio are stated for a face of any arity. An angle is unsigned and lies in `[0, pi]`, so a reflex corner of a non-convex face reads its explement. The input must be 3D.

### Dihedral Angles

`tf::compute_dihedral_angles` measures every edge two faces share — `edges` blocked in vertex pairs beside an aligned `angles` buffer, face-major:

```cpp [compute_dihedral_angles.cpp]
auto measured = tf::compute_dihedral_angles(polygons);

// With pre-computed manifold edge link and normals
auto tagged = polygons | tf::tag(mel) | tf::tag_normals(normals.unit_vectors());
auto measured_tagged = tf::compute_dihedral_angles(tagged);

for (std::size_t e = 0; e < measured.angles.size(); ++e) {
    auto edge  = measured.edges[e];    // [v0, v1]
    auto angle = measured.angles[e];   // tf::rad<T> between the face normals
}
```

One entry per undirected edge, reached through the manifold edge link's representative. The angle is between the face normals, so a flat surface reads `0`. A boundary or non-manifold edge joins no pair of faces and turns through no angle, so it is not stated. Manifold edge link and normals are taken from the form when tagged and built for the call when not.
...
bool oriented = tf::ensure_positive_orientation(polygons);

// Skip consistency step if mesh is already consistently oriented
oriented = tf::ensure_positive_orientation(polygons, true);
```
...
It returns `false` and does not take the volume flip when any component is non-orientable, since such a surface has no outward side. The orientable components are still made consistent.
...
`tf::triangulated` is the one triangulation entry. It takes an indexed polygon
mesh, a single `tf::polygon`, or a polygon soup, and always returns an owning
triangle mesh — the corners with the point table they index.
...
// A corner is a position in the returned point table, so the width it is
// written in is the caller's choice: ask for int32_t when an int64_t mesh
// does not need the range. It defaults to the input's own.
auto narrow = tf::triangulated<std::int32_t>(polygons);
```
...
A soup — `tf::make_polygons` over a range of polygons, where a corner two faces
share is stated once per face — triangulates through the same entry:

```cpp [triangulated_soup.cpp]
auto soup = tf::make_polygons(range_of_polygons);

// Index NAMES the output here: a soup carries no index type of its own, so
// the request is the caller's naming and it defaults to int.
auto triangle_mesh = tf::triangulated<std::int32_t>(soup);
```

::note{icon="i-lucide-info"}
A soup is [`tf::cleaned`](/cpp/modules/clean) to shared-vertex identity first, so the triangulation machinery only ever sees indexed meshes — a corner two faces share becomes one point, and the shared edge is one identity in both, exactly as for a mesh that arrived indexed. `tf::return_refused` takes indexed meshes only: the clean drops the faces that bound nothing, so a soup has no face identity to refuse.
::

::note{icon="i-lucide-info"}
Each face is triangulated on its own boundary as the constraint set, on the exact integer lattice and in the frame that face's own supporting corners give it. A shared mesh edge is **one identity** in both faces that carry it, so faces meeting on an edge stay watertight — including where a resolution splits that edge, since a split is a fact of the shared identity and reaches the neighbour with it. Triangle corners come out in the source polygon's winding.

The fill rule is `labels != 0`: a face's product is everything its triangulation kept inside its own boundary, and the hull exterior is the only thing dropped. For a simple loop that is the even-odd rule exactly; for a loop that overlaps itself it is not — a doubly wound region is inside, and this keeps it.
::

::note{icon="i-lucide-info"}
**A face that needs resolution is resolved, not dropped.** A loop that crosses itself states its crossing and **mints** the identity that names it, so the output point table is the input's own points followed by whatever this call created. A mesh no face of which needs resolving mints nothing, so its point table is the input's and its corners name only the input's own vertex ids.

A resolved face's position is unchanged, only its name: the election that closes an identity ranks an original vertex last, so an original coincident with another identity is retired into a minted one standing at exactly its own coordinate.

An original point is emitted through the form's frame and a minted one through the lattice the build quantized to, which puts both sides of the table in one space. On an untagged form the frame is the identity and the points are the input's, copied.
::

### Refused Faces

A face that holds no product is answered with emptiness. Pass `tf::return_refused` to also receive, ascending, the ids of the faces the triangulator refused:

```cpp [triangulated_refused.cpp]
// Returns std::pair<tf::polygons_buffer<Index, RealT, Dims, 3>, tf::buffer<Index>>
auto [triangle_mesh, refused] = tf::triangulated(polygons, tf::return_refused);
```
...
The mesh is exactly the one the untagged call returns. The ids are ascending face ids of the **input** — a different identity space from the mesh's corners — so they carry the input's own index type whatever width the mesh was asked for.

A face is refused when its triangulation refused **every round** of the resolution wave — it holds no product and its triangles are absent. A face the wave resolves is not refused, and a face that bounds no area holds its product by emitting nothing, on the same terms as every other carrier.
