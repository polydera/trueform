...
Boundary and non-manifold edges are excluded — see [Topology](/cpp/modules/topology) for [`make_boundary_edges`](/cpp/modules/topology#boundary-detection) and [`make_non_manifold_edges`](/cpp/modules/topology#non-manifold-edges). If manifold edge link or normals are not tagged, they are computed internally.
...
::note{icon="i-lucide-info"}
Uses the [Delaunay Triangulator](/cpp/modules/topology#delaunay-triangulator) internally — produces a constrained Delaunay triangulation per polygon. N-dimensional polygons are projected to 2D before triangulation.
::
