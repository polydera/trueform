...
To keep specific vertices through remeshing — registration landmarks, seams, constraint points — pass `tf::protect_vertices` with a per-vertex mask (a range of length `points().size()`, truthy where a vertex is pinned). A pinned vertex is never collapsed away and is frozen during relaxation. The call returns the new per-output-vertex **protection mask** marking where the pinned vertices landed; an empty input mask yields an empty mask (nothing pinned). This works across `tf::decimated`, `tf::simplified`, `tf::isotropic_remeshed`, and `tf::collapsed_short_edges`.

```cpp [protect_vertices.cpp]
std::vector<int> pin(polys.points().size(), 0);
pin[landmark_a] = 1;
pin[landmark_b] = 1;

auto [result, he, protection] =
    tf::simplified(polys, config, tf::protect_vertices(pin));
// protection[i] is true where output vertex i is one of the pinned vertices
```

Add `tf::return_index_map` to also receive the original → new **vertex index map**, so you can find where each pinned (or any surviving) vertex went:

```cpp [protect_vertices_map.cpp]
auto [result, he, protection, vertex_map] = tf::simplified(
    polys, config, tf::protect_vertices(pin), tf::return_index_map);

int new_id = vertex_map.f()[landmark_a]; // landmark_a's vertex id in result
```

The optional outputs always come in the order their tags were passed — `tf::preserve_regions` (face labels), then `tf::protect_vertices` (protection mask), then `tf::return_index_map` (index maps). So the fullest call returns all five:

```cpp [protect_regions_map.cpp]
auto [result, he, face_labels, protection, vertex_map] =
    tf::simplified(polys, config, tf::preserve_regions(region_of_face),
                   tf::protect_vertices(pin), tf::return_index_map);
```

::note{icon="i-lucide-info"}
The index map from `tf::return_index_map` is **vertices only** for `tf::simplified` and `tf::isotropic_remeshed` — their edge-flip (and, for isotropic, edge-split) passes rewrite face connectivity, so an original → final face map would be meaningless. The pure-collapse operations `tf::decimated` and `tf::collapsed_short_edges` return both a face and a vertex map. In every case a vertex collapsed away maps to a none sentinel; for `tf::isotropic_remeshed`, a split-created output vertex has no original preimage (its `kept_ids` entry is the none sentinel). `tf::return_index_map` can be used on its own, without `tf::protect_vertices`.
::
