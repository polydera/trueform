...
`tf::make_csg_graph` takes an optional [`tf::arrangement_config`](/cpp/modules/arrangement#arrangement-configuration) — the intersection settings plus the cut-surface triangulation, implicitly constructible from an [`tf::intersect_config`](/cpp/modules/intersect#intersection-configuration) alone, so every intersect-only spelling below stays valid. **The intersect mode decides what kinds of input are accepted and how degeneracies are resolved.** What works in CSG is exactly what works under that mode:

| Mode | What CSG accepts |
|--|--|
| `tf::intersect_mode::primitives` | Full 5-type intersection classification — handles shared edges, shared vertices, coplanar faces (aligned vs opposing boundary), and non-manifold edges shared by 3+ faces. The default. |
| `tf::intersect_mode::sos` | Symbolic-perturbation path: all intersections are edge-face, no degenerate cases. Faster, but no contact is ever coplanar — coplanar walls do not pool, and the domains they would have separated stay joined. |

Crossings between intersection curves that meet on the same face are resolved whenever the operands can make such a pair; there is no flag for it. `within` mode bit — canonically `primitives | within` — additionally self-arranges each operand: intersections **within** a form are found and cut alongside the pairwise ones. Spell it whenever an operand can self-overlap, e.g. two meshes concatenated into one operand. See [Self-Arrangements](#self-arrangements).

The default for `tf::make_csg_graph` is the bare `primitives`, matching `make_boolean`. Override only when you have a reason to:
...
The mode's semantics, including the `within` bit, the tolerance field, and the mode-vs-config equivalence are documented in [Intersect: Intersection Configuration](/cpp/modules/intersect#intersection-configuration).
...
### N forms with self-overlap: `within`

For multiple operands, self-intersections are skipped by default — a clean solid has none, and the pairwise sweep is cheaper without the self pass. When an operand can self-overlap — two meshes concatenated into one operand, an instanced part whose copies touch — ask the config for `within`:
...
The mesh overload takes an optional `tf::intersect_config` (default `primitives`; a one-form build implies `within`) for the self-arrangement. It builds its own arrangement, so it takes the lattice `Int` first and `OutputCoordinateType` second. An integral output keeps the shell on the exact lattice the arrangement ran on — nothing is deconverted — and the converter that produced it comes back with the mesh:
