...
`tf::make_boundary_rims` reads the same boundary as rims: per rim its vertex walk, the face carrying each of its edges, and whether it closes.

```cpp [boundary_rims.cpp]
auto rims = tf::make_boundary_rims(polygons);

// Or with pre-computed face membership
auto rims_tagged = tf::make_boundary_rims(polygons | tf::tag(face_membership));
auto rims_direct = tf::make_boundary_rims(polygons.faces(), face_membership);

for (std::size_t i = 0; i < rims.size(); ++i) {
    auto vertices = rims.vertices[i];   // the rim walked
    auto faces    = rims.faces[i];      // the face across each of its edges
    bool closed   = rims.closed[i];
}
```

Rim edge `k` runs from vertex `k` to vertex `k + 1` and is carried by face `k` alone, so a closed rim of `n` vertices has `n` edges — the last running back to vertex `0` — and an open one has `n - 1`.

A rim ends where the boundary stops passing straight through, so a vertex the boundary reaches more than twice closes one rim and starts the next, and a pinched boundary comes back as its pieces rather than as one figure eight. The mesh decides the rest: the boundary edges are its own order and the walk takes them in it, reaching a rim with two ends before the closed ones. A rim keeps the walk's direction when the edge it was entered by runs the way its face wound it and takes the opposite otherwise, so a consistently wound mesh gives rims that run the way its faces wind their boundary throughout.

::tip{icon="i-lucide-info"}
For the boundary as vertex paths alone — a pinch traversed rather than split — use `tf::make_boundary_paths()`. See [Boundary Detection](#boundary-detection).
::
...
`tf::make_non_manifold_vertices` names the vertices whose faces are not one fan — an edge at the vertex carries three or more faces, or its faces fall into several pieces meeting at the vertex alone:

```cpp [non_manifold_vertices.cpp]
auto vertices = tf::make_non_manifold_vertices(polygons);
// tf::buffer<Index> of vertex ids, ascending

// Or with pre-computed face membership
auto vertices_tagged = tf::make_non_manifold_vertices(polygons | tf::tag(fm));
auto vertices_direct = tf::make_non_manifold_vertices(polygons.faces(), fm);
```

The walk is on the face membership, so no half-edge structure is built. Winding is another fact and never enters the verdict, and a vertex no face names is not reported.

### Splitting Non-Manifold Vertices

`tf::split_non_manifold_vertices` gives every fan at a vertex a vertex of its own. The fan holding the vertex's smallest face keeps its id; every other fan takes a fresh id carrying the same coordinates, and that fan's corners are rewired onto it.

```cpp [split_non_manifold_vertices.cpp]
auto split = tf::split_non_manifold_vertices(polygons);
// tf::polygons_buffer with the input's faces and its points plus the mints

// With the point map: for each output point the input point it copies
auto [split_mesh, point_map] = tf::split_non_manifold_vertices(
    polygons, tf::return_index_map);
```

Faces keep their ids, their arity and their winding, and the points are the input's followed by the minted copies in id order.

An edge three faces carry is crossed by no fan, so the faces it holds are fans of their own and separating them would tear that edge into boundary copies. That is another verb's work: a vertex any of whose edges carries three faces is left exactly as it was, and `tf::make_non_manifold_vertices()` still names it. This one separates only the fans that come apart without cutting an edge.
...
`tf::is_manifold` is true when the faces around every vertex are one fan, which also says every edge carries at most two of them:

```cpp [is_manifold.cpp]
// Check if mesh is manifold
if (tf::is_manifold(polygons)) {
    // Every vertex one fan, so every edge on at most 2 faces
}

// Check if mesh has non-manifold features
if (tf::is_non_manifold(polygons)) {
    // An edge shared by 3+ faces, or a bowtie vertex
}
...
The question is the same one `tf::make_non_manifold_vertices()` answers per vertex, asked on the face membership and stopped at the first vertex that fails it. Winding is a separate fact: a mesh whose faces disagree about it is manifold all the same — see [`tf::orient_faces_consistently`](#face-orientation).

::tip{icon="i-lucide-info"}
To extract the offending features, use `tf::make_non_manifold_edges()` or `tf::make_non_manifold_vertices()`. See [Non-Manifold Edge Detection](#non-manifold-edge-detection) and [Non-Manifold Vertex Detection](#non-manifold-vertex-detection).
::
