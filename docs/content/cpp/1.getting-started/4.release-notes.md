...
One API change and a diagnostics family. **The intersection request is
two facts**: `intersect_mode` carries the classifier (`primitives` or
`sos`) and the `within` flag, spelled `primitives | within`; the resolve
flags (`resolve_crossing_contours`, `resolve_self_crossing_contours`,
`resolve_contours`, `self_intersections`) are removed. Crossings between
contour classes resolve unconditionally — every default already behaved
so and keeps its result byte for byte; the off-switch spellings no
longer exist. TypeScript curve entries now honor `within` (previously
stripped). **The mesh states its own health**:
`compute_face_quality`, `compute_dihedral_angles`, `make_boundary_rims`,
`make_non_manifold_vertices`, `is_manifold`,
`split_non_manifold_vertices` (with `return_index_map`), and
`has_self_intersections` — at all four tiers: core, the compiled facade
sync and async, Python, and TypeScript. **Domain membership counts
winding**: an N-operand `within` build no longer erases operand
membership in domain extraction (every bounded domain read "inside
operand 0" in 0.10.3/0.10.4) — the inclusion counts winding instead of
parity, so a double-covered pocket reads inside its own operand and a
zero-thickness fold encloses nothing, with clean inputs byte-identical;
and a multi-operand `within` build names self split points in the flat
vertex space, so a self-intersecting operand beside others cuts exactly
where it crosses itself instead of exhausting the recovery wave.
**Repairs**: a remesh under
machine load answers the mesh, not the schedule (identity minted in
input order, the collapse partition frozen before any task runs, the
vertex fan guard closing at its own vertex); binary STL reads are no
longer undefined on misaligned records; the facade's csg index map hands
through each tag's uncut span as the core states it.

## v0.10.4

Two new tiers and a new spatial query. The **volume module**: dense
scalar fields with a two-type carrier (int16 samples on a float grid),
mesh and analytic SDF generation — exact winding-counted sign, banded
mode 10-45x — field CSG with posed operands, resampling, plane
slicing, and extraction by Flying Edges or sharp dual contouring, with
the same surface in Python over zero-copy NumPy. The **compiled C++
facade** (`trueform/cpp`, `TF_BUILD_CPP=ON`): the library precompiled
over an explicit type matrix — mesh assembly with a stamped structure
cache, build verbs, the module surface sync and async, a VTK mesh
source. **Winding numbers**: `make_winding_moments` rides the existing
tree with order-two moments; `winding_number` answers single or batch,
and `signed_distance` on a winding-tagged form signs by it — the
graceful answer for soups and open meshes, and how both binding tiers
now sign. **NIfTI-1 IO** with `.nii.gz`, the affine split into grid
and pose, and hardware-CRC gzip validation. **Non-manifold edges are
sectored in every state**, making `make_outer_shell` idempotent on
pinched inputs and domain counts match the mesh-tier oracle.
