...
`trueform` is a C++ library for real-time geometric processing. Spatial queries, mesh booleans, isocontours, topology — at interactive speed on million-polygon meshes. Header-only; works directly on your data with zero-copy views. No copies, no conversions, no framework lock-in.
