...
`trueform` is a C++ library for real-time geometric processing. Arrangements, booleans, registration, remeshing and queries — at interactive speed on million-polygon meshes. Exact and robust to non-manifold flaps and pipeline artifacts. Header-only; works directly on your data with zero-copy views.
