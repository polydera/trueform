...
All examples are available in the [GitHub repository](https://github.com/xlabmedical/trueform/tree/main/vtk/examples).
...
cmake --build build-vtk --parallel --target vtk_examples
```
...
[Full source](https://github.com/xlabmedical/trueform/blob/main/vtk/examples/collision.cpp)
...
[Full source](https://github.com/xlabmedical/trueform/blob/main/vtk/examples/closest_pair.cpp)
...
[Full source](https://github.com/xlabmedical/trueform/blob/main/vtk/examples/boolean.cpp)
...
[Full source](https://github.com/xlabmedical/trueform/blob/main/vtk/examples/intersection_curves.cpp)
...
[Full source](https://github.com/xlabmedical/trueform/blob/main/vtk/examples/isocontours.cpp)

### Isobands

Alternating isoband regions with boundary curves. Hold Shift and scroll to animate the band selection—creates a marching-stripes effect across the surface. Uses the [`isobands`](/cpp/vtk/filters#isobands) filter with curve output.
...
[Full source](https://github.com/xlabmedical/trueform/blob/main/vtk/examples/isobands.cpp)
...
[Full source](https://github.com/xlabmedical/trueform/blob/main/vtk/examples/connected_components.cpp)
...
[Full source](https://github.com/xlabmedical/trueform/blob/main/vtk/examples/boundary_paths.cpp)

## Curvature Analysis

### Shape Index Histogram

Two-viewport display: mesh with highlighted neighborhood on the left, shape index histogram on the right. Move the mouse over the mesh—the neighborhood highlights and histogram updates in real-time. Uses [`compute_shape_index`](/cpp/modules/geometry#compute_shape_index) and [`neighborhood_applier`](/cpp/modules/topology#neighborhood_applier) for BFS traversal within a radius.

```cpp
// Compute shape index for all vertices
tf::buffer<float> shape_index;
shape_index.allocate(n_vertices);
tf::compute_shape_index(poly->polygons(), shape_index);

// On mouse move: collect neighborhood using BFS
...
// Color neighborhood vertices
auto neigh_colors = tf::make_indirect_range(current_indices_, colors_range);
tf::parallel_fill(neigh_colors, highlight);

// Compute histogram from neighborhood shape indices
auto neigh_si = tf::make_indirect_range(current_indices_, shape_index);
for (float si : neigh_si) {
  int bin = std::clamp(static_cast<int>((si + 1.0f) / bin_width), 0, num_bins - 1);
  histogram[bin]++;
}
```

[Full source](https://github.com/xlabmedical/trueform/blob/main/vtk/examples/shape_histogram.cpp)
