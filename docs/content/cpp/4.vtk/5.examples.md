...
Two-viewport display: mesh with highlighted neighborhood on the left, shape index histogram on the right. Move the mouse over the mesh—the neighborhood highlights and histogram updates in real-time. Uses [`compute_shape_index`](/cpp/geometry#compute_shape_index) and [`neighborhood_applier`](/cpp/topology#neighborhood_applier) for BFS traversal within a radius.
