...
Three-viewport display: original mesh on the left, decimated mesh (top-right) and isotropic remeshed mesh (bottom-right) with edge wireframe. A text strip at the bottom shows timing and face counts. Uses [`decimated`](/cpp/vtk/functions#decimated), [`mean_edge_length`](/cpp/vtk/functions#mean_edge_length), and [`isotropic_remeshed`](/cpp/vtk/functions#isotropic_remeshed).

```cpp
auto original = tf::vtk::read_stl("dragon-500k.stl");

// Decimate to 5% of original faces
auto vtk_dec = tf::vtk::decimated(original, 0.05f);

// Isotropic remesh to mean edge length of decimated result
float mel = tf::vtk::mean_edge_length(vtk_dec);
tf::isotropic_remesh_config<float> config{mel};
config.use_quadric = true;
auto vtk_rem = tf::vtk::isotropic_remeshed(vtk_dec, config);
```

[Full source](https://github.com/polydera/trueform/blob/main/vtk/examples/remeshing.cpp)
...
Split-screen viewer for volumetric domain extraction from a set of input OBJ files. Loads each `.obj`, computes their arrangement, labels per-face per-side domains, splits into watertight outward-oriented submeshes. Left / Right keys cycle through the domains; outer shell shown first, then by max-Z descending.

```bash
vtk_example_domains <dir>            # load every .obj from <dir>
vtk_example_domains a.obj b.obj ...  # load specific files
```

Pipeline spans arrangement, topology, and reindex: [`triangulated`](/cpp/modules/geometry#polygon-mesh) + [`cleaned`](/cpp/modules/clean#polygons) per input, then [`make_mesh_arrangements`](/cpp/modules/arrangement#mesh-arrangements), [`cleaned`](/cpp/modules/clean#polygons), [`make_domain_labels`](/cpp/modules/topology#volumetric-domains), and [`split_into_domains`](/cpp/modules/reindex#split-into-domains). Non-manifold edges are rendered via [`make_tube_mesh`](/cpp/modules/geometry#tube-mesh).

```cpp
// Per-input cleanup
auto raw = tf::read_obj<Index, Real>(path);
auto tri = tf::triangulated(raw.polygons());
auto m   = tf::cleaned(tri.polygons(), 1e-6);

// N-mesh arrangement + post-clean (welds intersection duplicates)
auto [mesh, _t, _f] = tf::make_mesh_arrangements(tf::make_range(forms));
mesh = tf::cleaned(mesh.polygons(), 1e-6);

// Volumetric domains: per-face per-side labels, then split into submeshes
...
// Outer shell first (most negative signed_volume), rest by max-Z desc
auto vol = tf::signed_volume(components[i].polygons());
auto z   = tf::aabb_from(components[i].polygons()).max[2];
```
...
[Full source](https://github.com/polydera/trueform/blob/main/vtk/examples/domains.cpp)
