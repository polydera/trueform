...
The core module bridges VTK and trueform. It provides zero-copy views that expose VTK data through trueform's semantic ranges—[`tf::points`](/cpp/modules/core#vectors-and-points), [`tf::polygons`](/cpp/modules/core#polygons), [`tf::curves`](/cpp/modules/core#paths-and-curves), and more—so you get the same ease of use and composability on VTK objects. Conversion utilities handle the reverse direction, turning trueform [buffers](/cpp/modules/core#data-structures-and-buffers) back into VTK objects. The [`tf::vtk::polydata`](#tfvtkpolydata) class extends `vtkPolyData` with cached acceleration structures for repeated queries.
...
Returns a [`tf::points`](/cpp/modules/core#vectors-and-points) view over the mesh vertices. Each element is a [`tf::point_view`](/cpp/modules/core#vectors-and-points).
...
Returns [`tf::edges`](/cpp/modules/core#with-edges) and [`tf::segments`](/cpp/modules/core#segments) views built from line data. Edges are pairs of vertex indices; each [`tf::segment`](/cpp/modules/core#segment) combines an edge with point coordinates.
...
Returns [`tf::unit_vectors`](/cpp/modules/core#vectors-and-points) views over normal data. Each element is a [`tf::unit_vector_view`](/cpp/modules/core#vectors-and-points). Empty if no normals are present.
...
Returns [`tf::aabb_tree`](/cpp/modules/spatial#tftree) structures for accelerated spatial queries.
...
| `half_edges()` | [`tf::half_edges`](/cpp/modules/topology#half-edges) | Half-edge structure for edge traversal |
...
|---------|-------|---------|---------|
| `make_points` | `vtkPoints*` or `vtkPolyData*` | `points_t` | [`tf::point_view`](/cpp/modules/core#vectors-and-points) |
...
| Factory | Input | Returns | Element |
|---------|-------|---------|---------|
| `make_point_normals` | `vtkPolyData*` | `normals_t` | [`tf::unit_vector_view`](/cpp/modules/core#vectors-and-points) |
| `make_cell_normals` | `vtkPolyData*` | `normals_t` | [`tf::unit_vector_view`](/cpp/modules/core#vectors-and-points) |
