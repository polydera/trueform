...
| `info.point` | [`tf::point`](/cpp/modules/core#vectors-and-points)`<float, 3>` | Closest point on the mesh |
...
| `info.metric` | `float` | Squared distance |
| `info.first` | [`tf::point`](/cpp/modules/core#vectors-and-points)`<float, 3>` | Closest point on first mesh |
| `info.second` | [`tf::point`](/cpp/modules/core#vectors-and-points)`<float, 3>` | Closest point on second mesh |
...
| `info.status` | [`tf::intersect_status`](/cpp/modules/core#ray-casting) | Intersection status |
| `info.t` | `float` | Parametric distance along ray |
| `info.point` | [`tf::point`](/cpp/modules/core#vectors-and-points)`<float, 3>` | Hit position |
| `operator bool()` | | True if intersection found |
...
Compute the oriented bounding box (minimum-volume enclosing box). Returns [`tf::obb`](/cpp/modules/core#oriented-bounding-box)`<float, 3>`.
...
Combine meshes using constructive solid geometry. See the [CSG module](/cpp/modules/csg#boolean-operations) for details on boolean operations and input requirements.

### make_boolean

Compute boolean operations between two meshes. Returns `vtkSmartPointer<polydata>` with "Labels" cell scalars indicating face origin (0 for mesh0, 1 for mesh1) and "Face Labels" cell scalars mapping each output face to the original face index in its source mesh.
...
With [`tf::return_curves`](/cpp/modules/iso#embedded-isocurves), returns `std::pair<vtkSmartPointer<polydata>, vtkSmartPointer<polydata>>` — the result mesh and intersection curves.
...
Orient face winding so adjacent faces have compatible normals. Modifies the input in place and returns `true` when every component is orientable and now consistent. Uses flood-fill through manifold edges; non-manifold edges act as barriers, and a component whose parity contradicts is left exactly as it was. See [Face Orientation](/cpp/modules/topology#face-orientation).

```cpp
bool orientable = tf::vtk::orient_faces_consistently(mesh);
```

## Scalar Field Operations

Extract features from scalar fields defined on mesh vertices. See the [Iso module](/cpp/modules/iso#embedded-isocurves) for the underlying algorithms.
...
Extract regions between scalar values. Returns `vtkSmartPointer<polydata>` with polygon cells, "BandLabel" cell scalars, and "Face Labels" cell scalars mapping each output face to the original face index. See [Isobands](/cpp/modules/iso#isobands).
...
With [`tf::return_curves`](/cpp/modules/iso#embedded-isocurves), returns `std::pair<vtkSmartPointer<polydata>, vtkSmartPointer<polydata>>` — the bands and their boundary curves.
...
Ensure faces are oriented with outward-pointing normals. First orients faces consistently, then flips all if signed volume is negative. Modifies the input in place and returns `true` when the mesh is now consistent and positively oriented; a surface carrying a non-orientable component has no outward side, so the volume flip is not taken. See [Ensure Positive Orientation](/cpp/modules/geometry#ensure-positive-orientation).

```cpp
bool oriented = tf::vtk::ensure_positive_orientation(mesh);

// Skip consistency step if already consistent
oriented = tf::vtk::ensure_positive_orientation(mesh, true);
```

### triangulated

Triangulate all polygons using constrained Delaunay triangulation. Returns a new polydata with triangle cells. See [Triangulation](/cpp/modules/geometry#triangulation).
...
Modify triangle mesh resolution through decimation, isotropic remeshing, and edge collapse. These functions wrap the [Remesh module](/cpp/modules/remesh). All assume triangle input and return a new polydata with cached half-edges.

::note{icon="i-lucide-info"}
All remesh functions require triangle meshes. Polygonal meshes must be triangulated first using [`triangulated`](#triangulated).
::

### decimated

Reduce face count using quadric error metrics. Returns `vtkSmartPointer<polydata>` with cached half-edges. See [Simplification](/cpp/modules/remesh#to-a-target-face-count).
...
See [`decimate_config`](/cpp/modules/remesh#with-configuration) for all parameters.

### isotropic_remeshed

Redistribute vertices to achieve uniform edge lengths. Returns `vtkSmartPointer<polydata>` with cached half-edges. See [Isotropic Remeshing](/cpp/modules/remesh#isotropic-remeshing).
...
// With configuration
tf::isotropic_remesh_config<float> config{2.0f};
config.iterations = 5;
config.use_quadric = true;
auto result = tf::vtk::isotropic_remeshed(mesh, config);
```

See [`isotropic_remesh_config`](/cpp/modules/remesh#with-configuration-1) for all parameters.

### collapsed_short_edges

Collapse edges shorter than a threshold. Unlike decimation which uses quadric error ordering, this collapses by edge length — shortest first. Returns `vtkSmartPointer<polydata>` with cached half-edges. See [Edge Collapse](/cpp/modules/remesh#edge-collapse).
...
See [`length_collapse_config`](/cpp/modules/remesh#with-configuration-2) for all parameters.
...
Remove duplicate points and degenerate faces from polygon data. Returns `vtkSmartPointer<polydata>`. See [Polygon Cleaning](/cpp/modules/clean#polygons).
...
Clean line data: merge duplicate points, remove degenerate edges, and reconnect edges into continuous paths. Returns `vtkSmartPointer<polydata>`. See [Curve Cleaning](/cpp/modules/clean#curves).
...
Remove duplicate points. Returns `vtkSmartPointer<vtkPoints>`. See [Point Cleaning](/cpp/modules/clean#points).
