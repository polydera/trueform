...
Reduce face count using quadric error metrics. Returns `vtkSmartPointer<polydata>` with cached half-edges. See [Decimation](/cpp/modules/remesh#decimation).
...
// With configuration
tf::remesh_config<float> config{2.0f};
config.iterations = 5;
config.use_quadric = true;
auto result = tf::vtk::isotropic_remeshed(mesh, config);
```

See [`remesh_config`](/cpp/modules/remesh#with-configuration-1) for all parameters.
