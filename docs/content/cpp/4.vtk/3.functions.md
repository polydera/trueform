...
Embed intersection curves from mesh1 into mesh0 without performing a boolean operation. Returns `vtkSmartPointer<polydata>` containing mesh0 with faces split along intersection curves and "Face Labels" cell scalars mapping each output face to the original face index. No faces from mesh1 are included. See [Embedded Intersection Curves](/cpp/modules/cut#embedded-intersection-curves).
...
Find where a mesh intersects itself and embed the intersection curves as edges. Returns `vtkSmartPointer<polydata>` with "Face Labels" cell scalars mapping each output face to the original face index. See [Embedded Self-Intersection Curves](/cpp/modules/cut#embedded-self-intersection-curves).
...
Extract regions between scalar values. Returns `vtkSmartPointer<polydata>` with polygon cells, "BandLabel" cell scalars, and "Face Labels" cell scalars mapping each output face to the original face index. See [Isobands](/cpp/modules/cut#isobands).
