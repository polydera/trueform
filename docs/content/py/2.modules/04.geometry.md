...
Measure every face of a mesh — one array per measure, each indexed by face:
...
quality  # array([1.], dtype=float32) — equilateral

# Also accepts a (faces, points) tuple
quality, min_angle, max_angle, aspect_ratio = tf.face_quality((faces, points))
```

| **Return** | **Shape / dtype** | **Description** |
|---|---|---|
| `quality` | `(num_faces,)`, mesh dtype | The triangle measure: `1` for equilateral, approaching `0` for a sliver; a face that is not a triangle has none and reads `-1` |
| `min_angle` | `(num_faces,)`, mesh dtype | Smallest corner angle, in radians |
| `max_angle` | `(num_faces,)`, mesh dtype | Largest corner angle, in radians |
| `aspect_ratio` | `(num_faces,)`, mesh dtype | Longest side over the shortest, infinite where a side has no length |

The corner angles and the aspect ratio are stated for a face of any arity. An angle is unsigned and lies in `[0, pi]`, so a reflex corner of a non-convex face reads its explement. 3D only.
...
edges   # array([[1, 2]], dtype=int32) — shape (num_edges, 2), each pair ascending
angles  # array([0.], dtype=float32)   — shape (num_edges,), radians
```

One entry per undirected edge two faces share, reached through the manifold edge link's representative. The angle is between the face normals, so a flat surface reads `0`. A boundary or non-manifold edge joins no pair of faces and turns through no angle, so it is not stated.

`edges` follows the face dtype, `angles` the point dtype. Accepts a `Mesh` or a `(faces, points)` tuple; on a `Mesh` the `manifold_edge_link` and `normals` it reads are built once and cached there. 3D only.
