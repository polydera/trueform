...
The spatial tree is built automatically on first query. Call `form.build_tree()` to prebuild it if you want to control when the cost is paid. `mesh.build_winding_moments()` does the same for the winding moments `tf.signed_distance` signs by — they mirror the tree's nodes and are invalidated with it.
::
...
`tf.signed_distance(mesh, query)` computes the distance from a point query to the closest point on a 3D mesh surface, signed by the generalized winding number: **negative inside, positive outside**. Either argument order is accepted.

```python
d = tf.signed_distance(mesh, [0.0, 0.0, 0.0])   # scalar, negative inside

# Batch → array of signed distances
pts = tf.Point(np.random.rand(1000, 3).astype(np.float32))
d = tf.signed_distance(mesh, pts)               # shape (1000,), mesh dtype
```

The mesh must be 3D with outward-oriented faces. The query is a `Point` (single or batch) or an array-like of shape `(3,)` or `(N, 3)` — array-likes are converted to the mesh's dtype.

The sign is a floating-point test on the winding number, not an exact predicate: robust in practice — including on open meshes and soups, where it degrades gracefully — but a query exactly on the surface may land on either side.

The winding number is read off the winding moments of the spatial tree, so the sign stays graceful on open meshes and soups. The moments build on first query; call `mesh.build_winding_moments()` to prebuild them.

| **Mesh** | **Query** | **Returns** |
|---|---|---|
| form | point | `float` |
| form | batch(N) | `ndarray(N,)` |

### Winding Number

`tf.winding_number(mesh, query)` is the generalized winding number itself — approximately 1 inside a closed surface, 0 outside, and a graceful fractional value for open sheets and soups. It reads the same winding moments `tf.signed_distance` signs by: far tree nodes answer by their stored expansion, near leaves by exact solid angles.

```python
w = tf.winding_number(mesh, [0.0, 0.0, 0.0])    # ~1 inside, ~0 outside

# Batch → array of winding numbers
pts = tf.Point(np.random.rand(1000, 3).astype(np.float32))
w = tf.winding_number(mesh, pts)                # shape (1000,), float64

# beta trades accuracy against traversal depth (default 2)
w = tf.winding_number(mesh, [0.0, 0.0, 0.0], beta=8.0)
```

The mesh must be 3D. The query is a `Point` (single or batch) or an array-like of shape `(3,)` or `(N, 3)`. The result is always float64 whatever the mesh's dtype — the number is dimensionless, not a coordinate.

Near the surface the value crosses 0.5 continuously, so any threshold on it is a float test. A closed input's exact inside/outside belongs to the exact predicates; this query belongs to the inputs they refuse.

| **Mesh** | **Query** | **Returns** |
|---|---|---|
| form | point | `float` |
| form | batch(N) | `ndarray(N,)` (float64) |
