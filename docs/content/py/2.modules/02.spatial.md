...
`tf.signed_distance(mesh, query)` computes the distance from a point query to the closest point on a 3D mesh surface, signed by the pseudonormal convention: **negative inside, positive outside**. Either argument order is accepted.
