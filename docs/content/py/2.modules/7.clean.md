...
Clean polygon or segment "soups" (unindexed geometry where each element stores its own vertex coordinates):
...
# Converts to indexed format, deduplicating shared vertices
faces, points = tf.cleaned(triangle_soup)

# Segment soup: shape (N, 2, Dims)
