...
`tf.cleaned` accepts two keyword-only flags after `*` for fine-grained control over which optional passes run. Vertex deduplication and degenerate-primitive removal **always** run; the flags only gate the two optional passes:
...
clean_faces, clean_points = tf.cleaned(
    (faces, points),
...
::note{icon="i-lucide-info"}
The flags apply to segment and polygon cleaning. For point cleaning and triangle/segment soups they're accepted for API uniformity but have no effect (neither concept applies).
::
...
- **`f`**: Forward mapping array of length N (original count)
  - `f[old_id]` gives the new index for kept elements (`< len(f)`)
  - `f[old_id] == len(f)` is the **sentinel** marking removed elements (degenerate, dropped duplicate, or unreferenced)
- **`kept_ids`**: Array of original indices that were kept, length M (new count)

The sentinel convention is uniform across the trueform clean module: any input whose `f[i] == len(f)` was removed; all survivors (including duplicates folded into a representative) satisfy `f[i] < len(f)`.
