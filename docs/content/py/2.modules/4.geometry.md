...
Compute the mean edge length of a polygon mesh:

```python
# From Mesh object
mel = tf.mean_edge_length(mesh)

# From (faces, points) tuple
mel = tf.mean_edge_length((faces, points))

# Dynamic polygon mesh (OffsetBlockedArray)
mel = tf.mean_edge_length((dyn_faces, points))
```
...
| `mean_edge_length` | `Mesh` or `(faces, points)` tuple | Mean edge length across all faces |
