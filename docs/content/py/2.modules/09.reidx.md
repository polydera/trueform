...
`tf.split_into_domains` is the per-side counterpart of `tf.split_into_components`, keyed by the tuple returned by `tf.domain_labels` (see [Topology module](/py/modules/topology)). Each face contributes up to two emissions, one per side; sentinel-bound sides are dropped, and side-0 emissions reverse the stored winding so every output submesh has outward-of-domain normals.

```python
mesh = tf.Mesh(faces, points)

# 1. Compute per-face per-side domain labels.
dl = tf.domain_labels(mesh, ignore_open_fragments=True)

# 2. Split into one watertight submesh per domain.
components, comp_labels = tf.split_into_domains(mesh, dl)

# components: list of (faces, points) tuples — one per domain, outward-oriented
# comp_labels: ndarray of domain ids (one per emitted submesh)
for (sub_faces, sub_points), domain_id in zip(components, comp_labels):
    print(f"domain {domain_id}: {len(sub_faces)} faces, {len(sub_points)} points")
```

The `domain_labels` argument accepts either the 3-tuple returned by `tf.domain_labels` or a bare `(labels, n_domains)` pair — `outer_shell_label` is not consumed by the split.

Works with both triangle meshes and dynamic n-gon meshes (`OffsetBlockedArray`); the output submeshes mirror the input layout.

::tip{icon="i-lucide-info"}
**Relationship with `split_into_components`**:
- `tf.split_into_components`: per-face label → one submesh per surface component.
- `tf.split_into_domains`: per-face-per-side label → one submesh per 3D domain, with winding fixed so normals point outward of that domain.
::
