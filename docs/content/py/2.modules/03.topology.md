...
Read the same boundary as rims — per rim its vertex walk, the face carrying each of its edges, and whether it closes:

```python
mesh = tf.Mesh(faces, points)

vertices, rim_faces, closed = tf.boundary_rims(mesh)
# vertices  : OffsetBlockedArray — block i is rim i walked
# rim_faces : OffsetBlockedArray — block i names the face across each of its edges
# closed    : ndarray of shape (R,), dtype int8 — nonzero where rim i closes

for rim_id in range(len(vertices)):
    rim = vertices[rim_id]              # vertex indices in walk order
    carriers = rim_faces[rim_id]        # one face per rim edge
    is_closed = bool(closed[rim_id])
```
...
A rim ends where the boundary stops passing straight through, so a pinched boundary comes back as its pieces rather than as one figure eight, and a rim with two ends is reached before the closed ones. A consistently wound mesh gives rims that run the way its faces wind their boundary.

Takes a `tf.Mesh` (triangle or dynamic) and reads its `face_membership`, building it on the mesh when it is not there yet.

::tip{icon="i-lucide-info"}
For the boundary as vertex paths alone — a pinch traversed rather than splitting the path — use `tf.boundary_paths(mesh)`.
::
...
Name the vertices whose faces are not one fan — an edge at the vertex carries three or more faces, or its faces fall into several pieces meeting at the vertex alone (a bowtie):

```python
# Two triangles meeting at a single vertex
...
# array([2], dtype=int32) — shape (N,), ascending, dtype matches faces.dtype
```

The walk is on the mesh's `face_membership`, so no half-edge structure is built. Face winding does not enter the verdict, and a vertex no face names is not reported. Takes a `tf.Mesh` (triangle or dynamic).

### Splitting Non-Manifold Vertices

Give every fan at a vertex a vertex of its own. The fan holding the vertex's smallest face keeps its index; every other fan takes a fresh index carrying the same coordinates, and that fan's corners are rewired onto it:

```python
split, point_map = tf.split_non_manifold_vertices(mesh)
# split     : Mesh — same face-index and point dtypes as the input
# point_map : ndarray of shape (P,) — for each output point the input
#             point it copies; an original point maps to itself

split.number_of_points   # 6 for the bowtie above
point_map                # array([0, 1, 2, 3, 4, 2], dtype=int32)
tf.is_manifold(split)    # True
```

Faces keep their ids, their arity and their winding, and the points are the input's followed by the minted copies.

An edge three or more faces carry is crossed by no fan, and separating the fans it holds apart would tear that edge into boundary copies. That is another verb's work: a vertex any of whose edges carries 3+ faces is left exactly as it was, and `tf.non_manifold_vertices` still names it. This one separates only the fans that come apart without cutting an edge.
...
if tf.is_manifold(mesh):
    ...
    print("Mesh has non-manifold features")

# is_non_manifold is the inverse of is_manifold
if tf.is_non_manifold(mesh):
    print("Mesh has a 3+ face edge or a bowtie vertex")
```

It is the question `tf.non_manifold_vertices` answers per vertex, asked on the mesh's `face_membership` and stopped at the first vertex that fails it. Non-manifold edges typically occur at T-junctions where three or more faces meet at a single edge; non-manifold vertices where two fans touch at a single point. Face winding does not enter the verdict.

::tip{icon="i-lucide-info"}
Use `tf.non_manifold_edges(mesh)` or `tf.non_manifold_vertices(mesh)` to get the offending features when `is_non_manifold` returns `True`, and `tf.split_non_manifold_vertices(mesh)` to separate the fans that come apart without cutting an edge.
::

### Euler Characteristic

`tf.euler_characteristic(mesh)` computes `V - E + F` — vertices minus unique edges plus faces:

```python
mesh = tf.Mesh(*tf.make_sphere_mesh(1.0))

chi = tf.euler_characteristic(mesh)   # 2
```

Each undirected edge is counted exactly once, so boundary and non-manifold edges count like interior ones: a closed sphere gives 2, a torus 0, a single triangle 1, an open disk-like patch 1. `V` counts every point in the array, including unreferenced ones. Works on triangle and dynamic meshes; also accepts a `(faces, points)` tuple.
...
### Refusing to split crossing constraints

By default a crossing between two constraints is resolved where the constraint walk meets it, adding an output point there. Pass `split_constraints=False` to demand the constraints verbatim instead: the triangulation then refuses on the first crossing and returns empty `faces` and `points`. That emptiness is the answer to "do these constraints cross" — it is not an error, and constraints that don't cross triangulate exactly as they do by default.

```python
edges = np.array([[0, 2], [1, 3]], dtype=np.int32)   # the two diagonals cross

faces, out_points = tf.cdt(points, edges, split_constraints=False)
crossing_free = faces.shape[0] > 0                    # False here
```

`split_constraints` needs `edges` — without constraints there is nothing to preserve, so passing it alone raises `ValueError`.

### Region labels

By default only the interior triangles (odd nesting parity) are returned. Pass `region_labels="nesting"` or `region_labels="components"` to instead get the **whole** triangulation — every triangle, hull exterior included — together with a per-triangle label:

- `"nesting"` — the parity of the constraint walls crossed from outside: `0` outside, `1` interior. `faces[labels == 1]` are the default entry's triangles, here over the full (uncompacted) point set.
- `"components"` — the id of the wall-cut component, `0` the hull exterior. Components tell regions apart that parity flattens together: an island nested inside a hole gets its own id.

```python
faces, out_points, labels = tf.cdt(points, edges, region_labels="nesting")
interior = faces[labels == 1]          # the default entry's triangles

faces, out_points, labels = tf.cdt(points, edges, region_labels="components")
```

`region_labels` requires `edges` — the labels state which side of the constraint walls a triangle lies on, and without constraints there are no walls to label. Labels are `int32`, one per output triangle. With `return_index_map=True` the return shape is `((faces, out_points), labels, (f, kept_ids))`. A `split_constraints=False` refusal answers with empty faces **and** empty labels.
