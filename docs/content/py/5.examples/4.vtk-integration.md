...
def on_interaction():
    ...
    (faces, points), labels, face_labels, (paths, curve_pts) = (
        tf.boolean_difference(mesh1, mesh2, return_curves=True)
    )
...
- Scalar field from plane distance with `tf.distance`
...
**Source:** [vtk/domains_from_files.py](https://github.com/polydera/trueform/tree/main/python/examples/vtk/domains_from_files.py)

Split-screen viewer for volumetric domain extraction. Loads one or more OBJ files, computes their arrangement, labels per-face per-side domains, and lets you cycle through the resulting watertight submeshes. Outer shell is shown first; remaining domains are ordered top→bottom by their max-Z vertex.
...
- Loading meshes with `tf.read_obj` (double precision)
- Mesh arrangement with `tf.mesh_arrangements`
- Mesh cleanup with `tf.cleaned`
- Volumetric labeling with `tf.domain_labels` (with `ignore_open_fragments`)
- Domain extraction with `tf.split_into_domains`
- Non-manifold-edge visualization via `tf.connect_edges_to_paths` + `tf.make_tube_mesh`
...
meshes = [tf.Mesh(*tf.read_obj(p, ngon=3, dtype=np.float64)) for p in paths]

# Arrangement of N meshes (intersections resolved, faces split).
(af, ap), _tags, _faces = tf.mesh_arrangements(meshes)
af, ap = tf.cleaned((af, ap), tolerance=1e-6)
arrangement = tf.Mesh(af, ap)

# Per-face per-side domain labels.
dl = tf.domain_labels(arrangement, ignore_open_fragments=True)

# Watertight outward-oriented submesh per domain.
...
```python [order.py]
# Outer shell (most negative signed_volume) first, then by max-Z descending.
...
other_idx = np.array([i for i in range(len(components)) if i != outer_idx])
...
### Non-manifold Edge Tubes

```python [nm_tubes.py]
nm_edges = tf.non_manifold_edges(arrangement)
if len(nm_edges) > 0:
    paths = tf.connect_edges_to_paths(nm_edges)
    radius = max(np.linalg.norm(points.max(0) - points.min(0)) * 0.001, 1e-6)
    tube_faces, tube_points = tf.make_tube_mesh(
        (paths, arrangement.points), radius=radius, radial_segments=12,
    )
```

::tip{icon="i-lucide-zap"}
The arrangement + domain pipeline preserves the full 2D structure of intersection contours, so the resulting submeshes are watertight along all shared interfaces.
::
...
**Source:** [vtk/remeshing.py](https://github.com/polydera/trueform/tree/main/python/examples/vtk/remeshing.py)
...
- Decimation with `tf.decimated`
- Mean edge length with `tf.mean_edge_length`
- Isotropic remeshing with `tf.isotropic_remeshed`
- Adaptive edge wireframe scaling
