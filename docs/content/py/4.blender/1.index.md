...
For scripts that process meshes without live preview, use `bpy.convert` to convert between formats and call trueform functions directly:
...
mesh_a = bpy.convert.from_blender(obj_a)
mesh_b = bpy.convert.from_blender(obj_b)
...
new_obj = bpy.convert.to_blender(result, name="Result")

# Optionally create curves object
curves_obj = bpy.convert.to_blender_curves(paths, curve_points, name="Curves")
```

### Add-ons with Live Preview

For add-ons that need cached meshes with automatic updates, use `bpy.scene`:

```python
from trueform import bpy

def register():
    bpy.register()  # Register scene handlers

def unregister():
    bpy.unregister()  # Clean up handlers

# In operator/handler - uses cached meshes with world transforms
result_obj = bpy.scene.boolean_difference(obj_a, obj_b, name="Result")

# Or with intersection curves
result_obj, curves_obj = bpy.scene.boolean_union(obj_a, obj_b, return_curves=True)

# Low-level access to cached mesh
mesh = bpy.scene.get(obj)  # tf.Mesh with world transform applied
```
