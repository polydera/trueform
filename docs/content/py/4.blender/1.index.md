...
# Boolean difference - returns (faces, points), labels, and face_labels
# labels indicates which input mesh (0 or 1) each face originated from
# face_labels maps each output face to its original face index
(result_faces, result_points), labels, face_labels = tf.boolean_difference(mesh_a, mesh_b)
