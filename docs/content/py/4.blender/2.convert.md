...
For live preview systems, update existing objects without recreating them.
...
Preserves object name, materials, display settings, and custom properties.

**Parameters:**

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `mesh` | `tf.Mesh` | | New geometry |
| `obj` | `bpy.types.Object` | | Existing object to update |
| `flat_shading` | `bool` | `True` | Use flat shading |

### update_curves

Update an existing Blender curves object with new paths:
...
paths, points = tf.intersection_curves(mesh_a, mesh_b)
tbpy.convert.update_curves(paths, points, existing_curves_obj)
```

Preserves curve name, bevel settings, materials, and fill caps.

**Parameters:**

| Parameter | Type | Description |
|-----------|------|-------------|
| `paths` | `OffsetBlockedArray` | New paths |
| `points` | `np.ndarray` | (N, 3) point coordinates |
| `obj` | `bpy.types.Object` | Existing curves object |
...
(faces, points), labels, face_labels, (paths, curve_points) = tf.boolean_difference(
...
For cached mesh access in add-ons with live preview, see [Scene](/py/blender/scene). For boolean operations on `tf.Mesh` objects, see [CSG](/py/modules/csg#boolean-operations).
::
