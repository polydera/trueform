...
The `bpy.convert` module provides utilities to convert between Blender mesh data and trueform `tf.Mesh` objects. Use these for standalone scripts where you don't need caching.
...
mesh = bpy.convert.from_blender(obj.data)
```
...
obj = bpy.convert.to_blender(mesh, name="Result")

# With smooth shading
obj = bpy.convert.to_blender(mesh, name="Result", flat_shading=False)
```
...
curves_obj = bpy.convert.to_blender_curves(paths, points, name="Curves")
```
...
```python
points, faces = bpy.convert.extract_geometry(mesh_data)
...
```python
mesh_data = bpy.convert.make_polygons(faces, points, name="Mesh")
...
```python
curve_data = bpy.convert.make_curves(paths, points, name="Curves")
...
result_obj = bpy.convert.to_blender(result, name="Boolean_Result")

# Create intersection curves
if len(paths) > 0:
    curves_obj = bpy.convert.to_blender_curves(paths, curve_points, name="Intersection_Curves")
