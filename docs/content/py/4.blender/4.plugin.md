...
## Installation
We provide pre-built add-on for Blender 5.0 and later (python 3.11 compatible) for the following platforms:
- Windows (64-bit)
- macOS (ARM)
- Linux (64-bit)

Download the latest release zip from the Trueform GitHub Releases page.
The file will be named `trueform-bpy-plugin-${OS}-py311.zip`.

### Enable the add-on in Blender:
1. Open Blender and go to Edit &rarr; Preferences &rarr; Add-ons
2. Click "Install...", select the downloaded ZIP file, and click "Install Add-on"
3. Enable the "Trueform Boolean" add-on from the list

> See the Blender [add-on docs](https://docs.blender.org/manual/en/latest/editors/preferences/addons.html) for more details on usage.

### Building custom add-on
To build the add-on from source, ensure you a python version compatible with your Blender version (e.g. python 3.11 for Blender 5.0). Then build trueform with the correct cmake flags:

```bash
cmake -B build -DTF_BUILD_PYTHON=ON -DTF_BUILD_PYTHON_BLENDER_INTEGRATION=ON -DCMAKE_BUILD_TYPE=Release
cmake --build build --target build_blender_plugin
```

Created blender zip can be found in `build/python/trueform-bpy-plugin.zip`.
