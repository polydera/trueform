...
```python
from trueform import bpy as tfb
...
This enables `tfb.scene.get()` to return cached meshes with automatic dirty tracking.
...
def _update_preview(context):
    ...
    if len(paths) > 0:
        ...
        if not existing:
            curves_obj = tfb.convert.to_blender_curves(paths, points, "Preview_Curves")
            _PREVIEW_CURVES_NAME = curves_obj.name
        else:
            # Reuse existing object, just update curve data
            old_data = existing.data
            existing.data = tfb.convert.make_curves(paths, points, "Preview_Curves")
...
class MESH_OT_boolean(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        op_map = {
            'DIFFERENCE': tfb.scene.boolean_difference,
            'UNION': tfb.scene.boolean_union,
            'INTERSECTION': tfb.scene.boolean_intersection
        }
