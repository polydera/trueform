...
::card-group
  ...
  icon: i-lucide-box
  title: PyVista
  to: https://github.com/polydera/pyvista-trueform
  target: _blank
  ---
  trueform's CSG engine behind a `.trueform` accessor on PyVista meshes.
  :::
...
**Remeshing** — decimate, simplify, and remesh:

```python
# Decimate to 50% of the faces
dec_faces, dec_points = tf.decimated(mesh, 0.5)

# Simplify to an error budget (flat regions collapse, detail survives)
sim_faces, sim_points = tf.simplified(mesh, error_rel=0.005)
