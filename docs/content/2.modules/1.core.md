...
auto sidedness = tf::classify(point, plane);
auto containment = tf::classify(point, polygon);
...
// ray_cast returns (intersection_status, t)
auto r_cast = tf::ray_cast(ray, seg, tf::make_ray_config(1.f, 2.f));
if (r_cast) {
  auto [status, t] = r_cast;
}

// ray_hit returns (intersection_status, t, point)
