...
auto [result_mesh, labels] = tf::make_isobands<int>(
    polygons, scalar_field, tf::make_view(levels));
...
// Extract trueform result
auto [result_mesh, labels] = some_operation();
...
// Copy points
for (auto pt : result_mesh.points()) {
...
for (auto face : result_mesh.faces()) {
    ...
    for (auto vid : face) {
        cell->InsertNextId(vid);
    }
    vtk_cells->InsertNextCell(cell);
}

vtkNew<vtkPolyData> result_polydata;
result_polydata->SetPoints(vtk_points);
result_polydata->SetPolys(vtk_cells);
```
