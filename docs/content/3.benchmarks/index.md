...
For implementation details and source code, see the [comparison examples](/examples/comparisons).


## Spatial Trees and Queries

Comparing spatial acceleration structures and proximity queries against **nanoflann** and **CGAL**. See [k-NN queries](/examples/comparisons#k-nn-query-performance) and [collecting intersecting primitives](/examples/comparisons#collecting-intersecting-primitives) examples.
...
Extracting boundary and non-manifold edges against **VTK**. Demonstrates efficient topology analysis using trueform's manifold edge link data structure. See [feature edges example](/examples/comparisons#feature-edges).
...
Computing intersection curves between two meshes against **CGAL** and **VTK**. Performance advantage grows with mesh size, demonstrating superior scaling characteristics in spatial indexing and parallel intersection algorithms. See [intersection curves (CGAL)](/examples/comparisons#intersection-curves) and [intersection curves (VTK)](/examples/comparisons#intersection-curves-1) examples.
...
Computing boolean operations (union, intersection, difference) between two meshes against **CGAL**. Speedup ranges from 30x to 80x depending on mesh size and operation complexity. See [mesh booleans example](/examples/comparisons#mesh-booleans).
