...
const props = withDefaults(defineProps<{ title?: string, description?: string, headline?: string, lib?: 'cpp' | 'py' }>(), {
  title: 'title',
...
const libLabel = computed(() => {
  if (props.lib === 'cpp') return 'C++'
  if (props.lib === 'py') return 'PYTHON'
  return ''
})
...
const headlineText = computed(() => {
  ...
  if (libLabel.value) parts.push(libLabel.value)
...
<template>
  <div class="w-full h-full flex flex-col justify-center bg-[#020420]">
    ...
    <div class="pl-[100px]">
      <p
        v-if="headlineText"
        class="uppercase text-[24px] text-[#00d5be] mb-4 font-semibold"
      >
        {{ headlineText }}
      </p>
      <h1
