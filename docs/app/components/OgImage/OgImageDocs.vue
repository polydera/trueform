...
const props = withDefaults(defineProps<{
  ...
  colorMode: 'dark',
  theme: '#00D5BE',
  siteLogo: 'https://trueform.pages.dev/tf.png',
})
...
<template>
  <div
    ...
    :class="[
      colorMode === 'light' ? ['bg-white', 'text-gray-900'] : ['bg-neutral-900', 'text-white'],
    ]"
  >
    <div
      class="flex absolute top-0 right-[-115%]" :style="{
        width: '200%',
        height: '200%',
    ...
    <div class="h-full w-full justify-between relative">
      <div class="flex flex-row justify-between items-start">
        <div class="flex flex-col w-full max-w-[65%]">
          <h2 class="m-0 font-bold mb-[30px] text-[32px] uppercase" style="display: block; text-overflow: ellipsis;" :style="{ lineClamp: description ? 2 : 3 }">
            {{ headline }}
          </h2>
          <h1 class="m-0 font-bold mb-[30px] text-[64px]" style="display: block; text-overflow: ellipsis;" :style="{ lineClamp: description ? 2 : 3 }">
            {{ title }}
          </h1>
          <p
            v-if="description"
