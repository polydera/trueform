...
import mesh_intersection_curves from "../../../benchmarks/vtk_isocontours_many.json";
...
const triggers = {
  [GroupedBar.selectors.bar]: (d: any) => `<div class="flex flex-col gap-0.5">
    <div class="font-medium text-lg">${d.num_contours} number of cut-values</div>
...
<template>
  <div class="w-full unovis flex flex-col gap-2.5 items-center justify-center">
    <h2 class="text-xl font-medium text-center">
      Isocontours (multiple cut-values):<br />
    ...
    <VisXYContainer :y-domain="[0, 180]">
      <VisGroupedBar :data="dataS" :x="x" :y="y" :color="color" :duration="300" />
      <VisTooltip :triggers="triggers" />
      <VisAxis
        type="x"
        label="Number of Cut-Values"
        :tickFormat="(value: number) => dataS[value]?.num_contours || ''"
