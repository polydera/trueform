...
import mesh_intersection_curves from "../../../benchmarks/mesh_intersection_curves.json";
...
const y = [(d: any) => d.vtk, (d: any) => d.cgal];
const color = (_: any, i: number) => ["#00529b", "#fdff4e"][i];
const triggers = {
  [GroupedBar.selectors.bar]: (d: any) => `<div class="flex flex-col gap-0.5">
    <div class="font-medium text-lg">${d.triangles} triangles</div>
    <div><span class="text-[#00529b]">VTK:</span> ${d.vtk}x</div>
    <div><span class="text-[#fdff4e]">CGAL:</span> ${d.cgal}x</div>
  </div>`,
};
</script>
<template>
