...
import mesh_intersection_curves from "../../../benchmarks/knn_nanoflann.json";
...
const triggers = {
  [GroupedBar.selectors.bar]: (d: any) => `<div class="flex flex-col gap-0.5">
    <div class="font-medium text-lg">${d.neighbors} neighbors</div>
    <div><span class="text-[#a82d12]">nanoflann:</span> ${round(d.speedup)}x</div>
  </div>`,
};
</script>
<template>
  <div class="w-full unovis flex flex-col gap-2.5 items-center justify-center">
    <h2 class="text-xl font-medium text-center">
      KNN Queries :<br />
      Speed-up of <span class="text-primary font-bold">trueform</span> vs nanoflann
    </h2>
    ...
    <VisXYContainer :y-domain="[0, 2]">
      <VisGroupedBar :data="dataS" :x="x" :y="y" :color="color" :duration="300" />
      <VisTooltip :triggers="triggers" />
      <VisAxis
        type="x"
        label="Number of Neighbors"
        :tickFormat="(value: number) => dataS[value]?.neighbors || ''"
