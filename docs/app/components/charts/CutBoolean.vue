...
import data from "../../../benchmarks/boolean.json";
...
const color = (_: any, i: number) => ["#00d5be", "#6baaff", "#fdff4e", "#ff6b6b"][i];
...
const template = (d: any) => `<div class="flex flex-col gap-0.5">
    ...
    <div><span class="text-[#6baaff]">MeshLib:</span> ${round(d.meshlib)} ms</div>
    <div><span class="text-[#fdff4e]">CGAL:</span> ${round(d.cgal)} ms</div>
    <div><span class="text-[#ff6b6b]">libigl:</span> ${round(d.igl)} ms</div>
  </div>`;

const lastPoint = data[data.length - 1];
const maxY = Math.max(...data.flatMap((d: any) => [d.tf, d.meshlib, d.cgal, d.igl]));
  ...
  <div class="w-full unovis flex flex-col gap-2.5 items-center justify-center">
    ...
    <div class="flex gap-4 items-center justify-center flex-wrap">
      ...
      <div class="flex gap-1.5 items-center">
        ...
        <span class="text-sm">MeshLib</span>
      </div>
      <div class="flex gap-1.5 items-center">
