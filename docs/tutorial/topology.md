...
auto component_count = tf::label_connected_components<int>(
    ...
    // optionaly specify, as > 500 components are more efficiently
    // found using a sequenctial algorithm (default is parallel)
...
Find Eulerian paths in edge graphs:

```cpp
// From edge membership structure
tf::buffer<int> path_offsets;
tf::buffer<int> edge_ids;
tf::find_eulerian_paths(edges, edge_membership, path_offsets, edge_ids);

// From vertex link (finds paths in vertex graph)
tf::buffer<int> vertex_path_offsets;
tf::buffer<int> vertex_ids;
tf::find_eulerian_paths(vertex_link, vertex_path_offsets, vertex_ids);

// Process the paths using tf::make_offset_block_range
auto paths = tf::make_offset_block_range(path_offsets, edge_ids);
for (const auto& path : paths) {
    for (auto edge_id : path) {
        // Process edge in Eulerian path
    }
}
```
