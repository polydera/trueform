...
/// @brief Creates vtkCellArray from an int-indexed blocked_buffer (copies with
/// conversion to vtkIdType).
template <std::size_t V>
auto make_vtk_cells(const tf::blocked_buffer<int, V> &faces)
    -> vtkSmartPointer<vtkCellArray> {
  auto cells = vtkSmartPointer<vtkCellArray>::New();
  auto n = faces.size();
  if (n == 0)
  ...
  conn->SetNumberOfTuples(static_cast<vtkIdType>(V * n));
  auto *conn_ptr = static_cast<vtkIdType *>(conn->GetVoidPointer(0));
  tf::parallel_transform(faces.data_buffer(), tf::make_range(conn_ptr, V * n),
...
/// @brief Creates vtkCellArray from an int-indexed offset_block_buffer (copies
/// with conversion to vtkIdType).
