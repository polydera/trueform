...
from ._dispatch import GATHER_IDS_FORM_FORM
...
def _gather_ids(form: Any, query: Any, predicate: str = "intersects", distance: Optional[float] = None) -> np.ndarray:
    ...
    query : Point, Segment, Triangle, Polygon, Ray, Line, Mesh, EdgeMesh, or PointCloud
    ...
    # Handle swapped args: if form is a primitive and query is a form, swap them
    is_form_form = isinstance(form, (Mesh, EdgeMesh, PointCloud))
    is_form_query = isinstance(query, (Mesh, EdgeMesh, PointCloud))
    if not is_form_form and is_form_query:
    ...
    if form.dtype != query.dtype:
        raise TypeError(
            f"Dtype mismatch: form has {form.dtype}, query has {query.dtype}. "
            f"Both objects must have the same dtype.")

    if is_form_query:
        type_pair = (type(form), type(query))
    ...
    assert not query.is_batch, "gather_ids does not support batch queries"

    meta = extract_meta(form)
    suffix = build_suffix(meta)
    fn = getattr(_trueform.spatial, f"gather_ids_{meta.form_name}_fp_{suffix}")
    return fn(form._wrapper, query._wrapper, predicate, distance)
