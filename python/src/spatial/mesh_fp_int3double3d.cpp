/*
 * Copyright (c) 2025 XLAB
 * All rights reserved.
 *
 * This file is part of trueform (trueform.polydera.com)
 *
 * Licensed for noncommercial use under the PolyForm Noncommercial
 * License 1.0.0.
 * Commercial licensing available via info@polydera.com.
 *
 * Author: Ziga Sajovic
 */

#include "trueform/python/spatial/form_prim_dispatch.hpp"
#include "trueform/python/spatial/signed_distance.hpp"
#include "trueform/python/spatial/winding_number.hpp"
#include <trueform/python/spatial/mesh.hpp>

namespace tf::py {

auto register_mesh_fp_int3double3d(nanobind::module_ &m) -> void {
  register_form_prim_ops<mesh_wrapper<int, double, 3, 3>, 3, double>(
      m, "mesh", "int3double3d");
  register_mesh_signed_distance<mesh_wrapper<int, double, 3, 3>, 3, double>(
      m, "int3double3d");
  register_mesh_winding_number<mesh_wrapper<int, double, 3, 3>, 3, double>(
      m, "int3double3d");
}

} // namespace tf::py
