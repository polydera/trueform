...
auto register_mesh_fp_int3float3d(nanobind::module_ &m) -> void {
  register_form_prim_ops<mesh_wrapper<int, float, 3, 3>, 3, float>(
      m, "mesh", "int3float3d");
  register_mesh_signed_distance<mesh_wrapper<int, float, 3, 3>, 3, float>(
      m, "int3float3d");
  register_mesh_winding_number<mesh_wrapper<int, float, 3, 3>, 3, float>(
      m, "int3float3d");
}
