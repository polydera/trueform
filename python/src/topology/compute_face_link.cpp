/*
 * Copyright (c) 2025 Žiga Sajovic, XLAB
 * Licensed for noncommercial use under the PolyForm Noncommercial
 * License 1.0.0. Commercial licensing available via ziga.sajovic@xlab.si.
 * https://github.com/xlabmedical/trueform
 */

#include "trueform/python/topology/compute_face_link.hpp"
#include <nanobind/nanobind.h>
#include <nanobind/ndarray.h>

namespace tf::py {

auto register_topology_compute_face_link(nanobind::module_ &m) -> void {
  using namespace nanobind;

  // ==========================================================================
  // COMPUTE_FACE_LINK
  // Index types: int32, int64
  // Ngon: 3, 4
  // Total: 4 bindings
  // ==========================================================================

  // int32, ngon=3 (triangles)
  m.def(
      "compute_face_link_int_3",
      [](ndarray<numpy, int, shape<-1, 3>> faces,
         const offset_blocked_array_wrapper<int, int> &face_membership) {
        return compute_face_link<int, 3>(faces, face_membership);
      },
      arg("faces"), arg("face_membership"));

  // int32, ngon=4 (quads)
  m.def(
      "compute_face_link_int_4",
      [](ndarray<numpy, int, shape<-1, 4>> faces,
         const offset_blocked_array_wrapper<int, int> &face_membership) {
        return compute_face_link<int, 4>(faces, face_membership);
      },
      arg("faces"), arg("face_membership"));

  // int64, ngon=3 (triangles)
  m.def(
      "compute_face_link_int64_3",
      [](ndarray<numpy, int64_t, shape<-1, 3>> faces,
         const offset_blocked_array_wrapper<int64_t, int64_t> &face_membership) {
        return compute_face_link<int64_t, 3>(faces, face_membership);
      },
      arg("faces"), arg("face_membership"));

  // int64, ngon=4 (quads)
  m.def(
      "compute_face_link_int64_4",
      [](ndarray<numpy, int64_t, shape<-1, 4>> faces,
         const offset_blocked_array_wrapper<int64_t, int64_t> &face_membership) {
        return compute_face_link<int64_t, 4>(faces, face_membership);
      },
      arg("faces"), arg("face_membership"));
}

} // namespace tf::py
