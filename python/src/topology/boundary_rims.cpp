...
#include "trueform/python/topology/boundary_rims.hpp"
...
auto register_topology_boundary_rims(nanobind::module_ &m) -> void {
  ...
  m.def(
      ...
      [](ndarray<numpy, int, shape<-1, 3>> cells,
        ...
        return boundary_rims<int, 3>(cells, fm);
      },
      arg("cells"), arg("face_membership"));

  // int32, dynamic
  m.def(
      "boundary_rims_int_dyn",
      [](const offset_blocked_array_wrapper<int, int> &cells,
         const offset_blocked_array_wrapper<int, int> &fm) {
        return boundary_rims_dynamic<int>(cells, fm);
      },
      arg("cells"), arg("face_membership"));

  // int64, ngon=3
  m.def(
      "boundary_rims_int64_3",
      [](ndarray<numpy, int64_t, shape<-1, 3>> cells,
         const offset_blocked_array_wrapper<int64_t, int64_t> &fm) {
        return boundary_rims<int64_t, 3>(cells, fm);
      },
      arg("cells"), arg("face_membership"));

  // int64, dynamic
  m.def(
      "boundary_rims_int64_dyn",
      [](const offset_blocked_array_wrapper<int64_t, int64_t> &cells,
         const offset_blocked_array_wrapper<int64_t, int64_t> &fm) {
        return boundary_rims_dynamic<int64_t>(cells, fm);
      },
      arg("cells"), arg("face_membership"));
}
