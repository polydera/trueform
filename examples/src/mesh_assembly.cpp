...
int main() {
  ...
  tf::aabb_tree<int, float, 3> sheet_tree(cone_sheet.points(),
                                          tf::config_tree(4, 4));
  auto [center_id, _] =
      tf::neighbor_search(tf::make_form(sheet_tree, cone_sheet.points()),
  ...
  // === Alignment Test ===
  std::cout << "\n=== Alignment Test ===" << std::endl;

  // Create random point cloud X
  tf::points_buffer<float, 3> X;
  X.allocate(100);
  for (auto pt : X.points()) {
    auto rpt = tf::random_point<float, 3>();
    pt[0] = rpt[0];
    pt[1] = rpt[1];
    pt[2] = rpt[2];
  }

  // Create Y as transformed copy of X
  tf::vector<float, 3> translation{1.5f, -2.0f, 3.0f};
  auto T_ground_truth = tf::random_transformation(translation);
  // auto T_ground_truth = tf::transformed(
  //     tf::make_transformation_from_translation(translation),
  //     tf::transformed(
  //         tf::transformed(
  //             tf::make_rotation(tf::deg(tf::random(0.f, 360.f)), tf::axis<0>),
  //             tf::make_rotation(tf::deg(tf::random(0.f, 360.f)), tf::axis<1>)),
  //         tf::make_rotation(tf::deg(tf::random(0.f, 360.f)), tf::axis<2>)));

  tf::points_buffer<float, 3> Y;
  Y.allocate(X.points().size());
  for (auto [x, y] : tf::zip(X.points(), Y.points())) {
    auto tx = tf::transformed(x, T_ground_truth);
    y[0] = tx[0];
    y[1] = tx[1];
    y[2] = tx[2];
  }
  ...
  auto compute_error = [](const auto &A, const auto &B) {
    float max_err = 0.0f;
    for (auto [a, b] : tf::zip(A, B))
      max_err = std::max(max_err, tf::distance(a, b));
    return max_err;
  };

  // Make working copies
  tf::points_buffer<float, 3> X_rigid = X;
  tf::points_buffer<float, 3> X_obb = X;

  auto rigid_t = tf::make_identity_transformation<float, 3>();
  auto obb_t = tf::make_identity_transformation<float, 3>();

  std::cout << "Iter\tRigid Error\tOBB Error" << std::endl;

  for (int iter = 0; iter < 20; ++iter) {
    // Rigid alignment iteration
    auto T_rigid = tf::fit_rigid_alignment(X_rigid.points(), Y.points());
    rigid_t = tf::transformed(rigid_t, T_rigid);
    for (auto &&pt : X_rigid) {
      pt = tf::transformed(pt, T_rigid);
    }

    // OBB alignment iteration
    auto T_obb = tf::fit_obb_alignment(X_obb.points(), Y.points());
    obb_t = tf::transformed(obb_t, T_obb);
    for (auto &&pt : X_obb) {
      pt = tf::transformed(pt, T_obb);
    }

    float rigid_err = compute_error(X_rigid.points(), Y.points());
    float obb_err = compute_error(X_obb.points(), Y.points());

    std::cout << iter << "\t" << rigid_err << "\t\t" << obb_err << std::endl;
  }

  auto print_transform = [](const char *name, const auto &T) {
    std::cout << "\n" << name << ":" << std::endl;
  ...
  print_transform("Ground truth", T_ground_truth);
  print_transform("Rigid (accumulated)", rigid_t);
  print_transform("OBB (accumulated)", obb_t);
