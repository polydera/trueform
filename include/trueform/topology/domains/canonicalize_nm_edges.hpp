...
///      thirds (would-be degenerate cross product in int) by leaving
///      `is_valid[k] = 0`.
...
/// @tparam Edges Range of NM edges.
/// @tparam Faces Mutable offset-block range of incident faces per NM edge.
/// @tparam IdView, LabelsView Per-edge block ranges parallel to Faces.
...
void canonicalize_nm_edges(const Polygons &polygons,
  ...
  tf::parallel_for_each(
      tf::zip(nm_edges, nm_edge_faces, labels_view, id_sorted_view, is_valid),
      [&](auto t) {
        auto &&[edge, face_block, label_block, id_block, valid] = t;
