...
namespace tf {
template <typename Index> class face_paths_extractor {
public:
  template <typename Range, typename Policy>
  auto build(const Range &base_loop, const tf::edges<Policy> &edges,
...
private:
  ...
  auto build_connectivity(const tf::edges<Policy> &edges,
                          std::size_t n_vertices) {
    _v_link.build(edges, n_vertices, tf::edge_orientation::bidirectional);
