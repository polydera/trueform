...
public:
  ...
  auto build(const tf::polygons<Policy> &form,
    ...
    base_t::clear();
    _converter = make_vertex_converter<RealType>(form);
...
private:
  ...
  static auto convert_face(const Poly &poly, int tag, const Conv &conv,
                           tf::buffer<tf::exact::vertex<Index>> &out) {
    auto n = poly.size();
    out.reallocate(n);
  ...
  edges_vs_face_sos(const EdgePoly &edge_poly, const FacePoly &face_poly,
                    ...
                    const Conv &conv,
                    tf::buffer<tf::exact::vertex<Index>> &face_buf, Ints &ints,
                    Pts &pts, const tf::buffer<bool> &shared) {
  ...
  static auto primitives_polygon_pair(
    ...
    if ((mask0 & has_crossing) == has_crossing)
      crossing_edges_vs_face_self(face_buf0, n0, face_buf1, n1, signs0, tag0,
                                  tag1, face0_id, face1_id, is_rep0, is_rep1,
                                  ints, pts, both_crossing);
    if ((mask1 & has_crossing) == has_crossing)
      crossing_edges_vs_face_self(face_buf1, n1, face_buf0, n0, signs1, tag1,
                                  tag0, face1_id, face0_id, is_rep1, is_rep0,
                                  ints, pts, both_crossing);
    if (!any_zero)
      return;
    ...
    coplanar_primitives(face_buf0, n0, face_buf1, n1, signs0, signs1, tag0,
                        ...
                        plane1, shared0_f, shared1_f, ints, pts);
  ...
  template <typename Policy> auto build_sos(const tf::polygons<Policy> &form) {
    ...
    auto make_self_duplicator = [&](auto rec, auto &buffer) {
      duplicate_intersection(form.faces(), form.face_membership(),
  ...
  auto build_primitives(const tf::polygons<Policy> &form) {
    tf::local_buffer<intersection_t> l_intersections;
    tf::local_buffer<pt3> l_points;
    tf::local_buffer<tf::exact::vertex<Index>> l_face_verts0;
    tf::local_buffer<tf::exact::vertex<Index>> l_face_verts1;
    tf::local_buffer<bool> l_shared0;
    tf::local_buffer<bool> l_shared1;
    l_intersections.reserve_all(1000);
    l_points.reserve_all(1000);
    ...
    dedup_vertex_points(raw, points,
                        [&](Index, Index object, Index local_id) -> Index {
                          return Index(form.faces()[object][local_id]);
                        });
