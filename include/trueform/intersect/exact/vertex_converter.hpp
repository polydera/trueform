...
template <typename RealT, std::size_t Dims> struct vertex_converter {
  pt_converter<RealT, Dims> coords;
  ...
  template <typename P>
  auto convert(const P &p) const -> tf::point<int32_t, Dims> {
    return coords(p);
  }

  /// Reverse conversion (int32 back to float).
...
auto make_vertex_converter(tf::range<Iterator, N> forms) {
  if constexpr (std::is_same_v<RealT, tf::none_t>) {
    ...
    return make_vertex_converter<tf::coordinate_type<Policy>>(forms);
  } else {
    using form_t = std::decay_t<decltype(forms[0])>;
    constexpr auto Dims = tf::coordinate_dims_v<form_t>;
    auto conv = make_pt_converter<RealT>(forms);
    tf::small_vector<int, 4> offsets;
    int cumulative = 0;
    ...
    return vertex_converter<RealT, Dims>{conv, std::move(offsets)};
  }
}
