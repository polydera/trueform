...
void coplanar_primitives(
  ...
  for (std::size_t i = 0; i < n0; ++i) {
    ...
    for (std::size_t j = 0; j < n1; ++j) {
      ...
      // This verdict forms a shared identity — these records are closed by
      // a union-find into vertex classes — so it is EQUALITY, which is
      // transitive, and no chain can grow a class.
      if (!(sh_i && sh_j) && vrep_i && vrep_j && signs0[i] == 0 &&
          signs1[j] == 0 && face0[i].pt == face1[j].pt) {
      ...
      if (!sh_i && vrep_i && erep_j && signs0[i] == 0 &&
          ...
          !(face0[i].pt == face1[j].pt) && !(face0[i].pt == face1[nj].pt)) {
        if (tf::exact::orient2d_sign(face1[j], face1[nj], face0[i], plane1.ax0,
                                     plane1.ax1) == 0 &&
      ...
      if (!sh_j && vrep_j && erep_i && signs1[j] == 0 &&
          ...
          !(face1[j].pt == face0[i].pt) && !(face1[j].pt == face0[ni].pt)) {
        if (tf::exact::orient2d_sign(face0[i], face0[ni], face1[j], plane0.ax0,
                                     plane0.ax1) == 0 &&
      ...
      if (!((sh_i && sh_ni) || (sh_j && sh_nj)) && erep_i && erep_j &&
        ...
        if (tf::exact::segments_cross(p0i, p0ni, p1j, p1nj)) {
          emit_record(
