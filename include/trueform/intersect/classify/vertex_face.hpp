...
/// A vertex on any target edge yields sign 0 and is rejected here so it
/// falls through to the VE branch in `coplanar_primitives`.
...
void vertex_face(tf::exact::vertex_range<Index, Int> source_verts,
  ...
  for (std::size_t i = 0; i < n_source; ++i) {
    ...
    for (std::size_t k = 0; k < n_target && inside; ++k) {
      ...
      auto s = orient2d_sign(target_verts[k], target_verts[nk], source_verts[i],
                             ax0, ax1);
      if (s == 0)
        inside = false;
...
void vertex_face(tf::exact::vertex_range<Index, Int> source_verts,
                 ...
                 Intersections &intersections, Pts &pts) {
  vertex_face(
      source_verts, n_source, target_verts, n_target, source_signs, source_tag,
      target_tag, source_face_id, target_face_id, source_vrep,
      [](std::size_t) { return false; }, target_plane, intersections, pts);
}
