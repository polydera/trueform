...
auto make_polygons(Range0 &&faces, Range1 &&points) {
  ...
  return polygons<polygon_range<decltype(r0), decltype(r1)>>{
      polygon_range<decltype(r0), decltype(r1)>{r0, r1}};
}

template <typename Range0, typename Range1, typename Range2>
auto make_polygons(Range0 &&faces, Range1 &&points, Range2 &&normals) {
  auto r0 = tf::make_range(faces);
  auto r1 = tf::make_points(points);
  auto r2 = tf::make_unit_vectors(normals);
  return polygons<polygon_range<decltype(r0), decltype(r1), decltype(r2)>>{
      polygon_range<decltype(r0), decltype(r1), decltype(r2)>{r0, r1, r2}};
}
