...
auto closest_point_parametric(const tf::ray_like<Dims, Policy0> &ray0,
  ...
  auto [non_parallel, t0, t1] = tf::core::line_line_check(ray0, ray1);
...
/// segment.
template <std::size_t Dims, typename Policy, typename T>
auto closest_point_parametric(const tf::ray_like<Dims, Policy> &ray0,
...
auto closest_point_parametric(const tf::line_like<Dims, Policy> &line,
  ...
  auto [non_parallel, t0, t1] = tf::core::line_line_check(line, line_segment);
  using real_t = decltype(t0);
...
auto closest_point_parametric(const tf::line_like<Dims, Policy0> &line0,
  ...
  auto [non_parallel, t0, t1] = tf::core::line_line_check(line0, line1);
  using real_t = decltype(t0);
  if (!non_parallel) {
