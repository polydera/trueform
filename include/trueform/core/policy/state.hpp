 ...
 */
template <typename Index, typename Base> struct tag_state : Base {
   ...
   * @brief Constructs an instance.
   */
  tag_state(Index &&_state, Base &&base)
      : Base{std::move(base)}, _state{std::move(_state)} {}

  template <typename Other>
  auto operator=(Other &&other)
      -> std::enable_if_t<has_state_policy<Other> &&
   ...
   */
  auto state() const -> const Index & { return _state; }

  /**
   * @brief Returns a mutable reference to the state.
   */
  auto state() -> Index & { return _state; }
...
private:
  ...
  template <typename T> friend auto wrap_like(const tag_state &val, T &&t) {
    return tag_state<Index, std::decay_t<T>>{val.state(), static_cast<T &&>(t)};
  }
};
} // namespace policy
...
template <typename Proxy, typename Base> struct tag_state_ptr : Base {
   ...
   * @brief Constructs an instance.
   */
  tag_state_ptr(Proxy &&_state, Base &&base)
      : Base{std::move(base)}, _state{std::move(_state)} {}
...
/**
 * @ingroup injectors
 * @brief Constructs an `tag_state` by injecting i:d into a base
 */
template <typename Iterator, typename Base>
auto tag_state_ptr(Iterator &&state, Base &&base) {
  if constexpr (has_state_policy<Base>)
    if constexpr (std::is_rvalue_reference_v<Base &&>)
      return static_cast<Base>(base);
    else
...
struct state_ptr_tagger {
  ...
  auto operator()(std::pair<Iter0, Iter1> iters) const {
    return tf::tag_state_ptr(iters.first, *iters.second);
  }
};
...
template <typename U, typename T> auto operator|(U &&u, tag_state_op<T> t) {
  return tf::tag_state(t.state, static_cast<U &&>(u));
}
} // namespace policy
