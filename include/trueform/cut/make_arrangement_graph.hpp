...
#include "./dispatch/arrangement_pair_policy.hpp"
...
///        owns everything an extraction needs.
///
...
auto make_arrangement_graph(Forms in_forms,
  ...
  if constexpr (std::is_same_v<S, tf::none_t>) {
    return tf::arrangement_graph<tf::cut::arrangement_range_policy<Forms, S>,
                                 Int>(
        tf::cut::arrangement_range_policy<Forms, S>(std::move(forms),
                                                    tf::small_vector<S, 10>{}),
        config);
  } else {
...
auto make_arrangement_graph(const tf::polygons<Policy> &form,
  ...
  if constexpr (std::is_same_v<S, tf::none_t>) {
    using P =
        tf::cut::arrangement_range_policy<std::array<tf::polygons<Policy>, 1>,
                                          S>;
    return tf::arrangement_graph<P, Int>(
    ...
    using P =
        tf::cut::arrangement_range_policy<std::array<tf::polygons<Policy>, 1>,
                                          S>;
    return tf::arrangement_graph<P, Int>(P(std::move(forms), std::move(structs)),
                                         config);
  }
}
...
auto make_arrangement_graph(const tf::polygons<Policy0> &form0,
                            ...
                            tf::arrangement_config config = {}) {
  using S0 = decltype(tf::cut::dispatch::make_missing_structures(form0));
  using S1 = decltype(tf::cut::dispatch::make_missing_structures(form1));
  using P = tf::cut::arrangement_pair_policy<tf::polygons<Policy0>, S0,
                                             tf::polygons<Policy1>, S1>;
  if constexpr (std::is_same_v<S0, tf::none_t> &&
                std::is_same_v<S1, tf::none_t>) {
    return tf::arrangement_graph<P, Int>(P(form0, tf::none, form1, tf::none),
                                         config);
    ...
    S1 s1;
    tbb::parallel_invoke(
        [&] { s0 = tf::cut::dispatch::make_missing_structures(form0); },
        [&] { s1 = tf::cut::dispatch::make_missing_structures(form1); });
    return tf::arrangement_graph<P, Int>(
        P(form0, std::move(s0), form1, std::move(s1)), config);
  } else if constexpr (!std::is_same_v<S0, tf::none_t>) {
    auto s0 = tf::cut::dispatch::make_missing_structures(form0);
    return tf::arrangement_graph<P, Int>(
        P(form0, std::move(s0), form1, tf::none), config);
  } else {
    auto s1 = tf::cut::dispatch::make_missing_structures(form1);
    return tf::arrangement_graph<P, Int>(
