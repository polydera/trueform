...
/// index-map `end` convention: `point_tag_labels[o] == n_output_points` (one
/// past every real output index) marks a created point. Equivalently
/// `o >= n_original_points`; the created id is positional (`o -
/// n_original_points`).
///
...
template <typename Index> struct mesh_arrangement_index_map {
  /// output point -> input mesh tag; created -> n_output_points
  tf::buffer<Index> point_tag_labels;
  /// output point -> input point id (local to its mesh)
