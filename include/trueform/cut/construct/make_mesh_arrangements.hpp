...
///
/// Takes the arrangement's exposed triangle-grain loops and a range of
/// polygon forms. Returns (mesh, tag_labels, face_labels, map_data).
...
auto make_mesh_arrangements(
    const tf::cut::region_triangulator<Index, Int> &rt, const FormsRange &forms,
  ...
  // 1. Build maps. Promoted (conforming) faces carry descriptors, so
  // the map build already keeps them out of the uncut lists.
  auto map_data =
      tf::cut::make_arrangement_map_data(
          rt, apply_to_polygons, n_meshes,
          static_cast<Index>(created_pts.size()));

  // 2. Cut loops: one exposed triangle each, in loop order — winding
  // (coplanar folds) is already applied in the exposed stream.
  ...
  tf::buffer<Index> tri_origins;
  {
    ...
    tf::parallel_for_each(
        tf::make_sequence_range(n_tris), [&](std::size_t l) {
          const auto &desc = descs[Index(l)];
          const auto &tr = loops[Index(l)];
          for (int c = 0; c < 3; ++c)
            tri_data[3 * l + std::size_t(c)] =
                map_data.map_vertex(desc.tag, tr[std::size_t(c)]);
          tri_tags[l] = desc.tag;
          tri_origins[l] = desc.object;
        });
  }
  ...
  // 5. Build points (parallel per mesh + created points — the unified
  // table: intersection points followed by any refinement-added ones)
  auto total_pts =
      map_data.total_original_points + map_data.total_created_used;
...
auto make_mesh_arrangements(
...

  {
      ...
      if constexpr (std::is_integral_v<RealOut>) {
        tf::parallel_copy(
            tf::make_points(tf::make_indirect_range(
                ...
                tf::make_mapped_range(form0.points(),
                                      [frame, &converter](auto pt) {
                                        return converter.convert(
      ...
      if constexpr (std::is_integral_v<RealOut>) {
        tf::parallel_copy(
            tf::make_points(tf::make_indirect_range(
                ...
                tf::make_mapped_range(form1.points(),
                                      [frame, &converter](auto pt) {
                                        return converter.convert(
                                            tf::transformed(pt, frame));
                                      }))),
    ...
    tg.run([&] {
      ...
      if constexpr (std::is_integral_v<RealOut>) {
        tf::parallel_copy(tf::make_points(gathered),
                          tf::drop(pts_buf, map_data.total_original_points));
      } else {
        tf::parallel_copy(
            tf::make_points(tf::make_mapped_range(
...
/// 2-mesh rt overload: different form types, the exposed triangle
/// stream as the cut geometry — the graph-path successor of the fc
/// pair emitter above (same assembly, no emit-time CDT).
template <typename OutputCoordinateType = tf::none_t, typename Index,
          typename Int, typename Policy0, typename Policy1, typename RealType>
auto make_mesh_arrangements(
  ...
  // 1. Maps. Promoted (conforming) faces carry descriptors, so the map
  // build already keeps them out of the uncut lists.
  auto map_data =
      tf::cut::make_arrangement_map_data(
          rt, apply_to_polygons, Index(2),
          static_cast<Index>(created_pts.size()));

  // 2. Cut loops: one exposed triangle each, in loop order.
  tf::buffer<Index> tri_data;
  tf::buffer<Index> tri_tags;
  tf::buffer<Index> tri_origins;
  {
  ...
  // 3. Points: originals per form + the unified created table.
  auto total_pts =
      map_data.total_original_points + map_data.total_created_used;
  tf::points_buffer<RealOut, 3> pts_buf;
  pts_buf.allocate(total_pts);
  {
    auto pts_range =
        tf::make_offset_block_range(map_data.original_offsets, pts_buf);
    auto fill = [&](const auto &form, Index t) {
      auto frame = tf::frame_of(form);
      if constexpr (std::is_integral_v<RealOut>) {
        tf::parallel_copy(
            tf::make_points(tf::make_indirect_range(
                map_data.original_ids[std::size_t(t)],
                tf::make_mapped_range(form.points(),
                                      [frame, &converter](auto pt) {
                                        return converter.convert(
                                            tf::transformed(pt, frame));
                                      }))),
    ...
    tbb::task_group tg;
    tg.run([&] { fill(form0, Index(0)); });
    tg.run([&] { fill(form1, Index(1)); });
    tg.run([&] {
      if constexpr (std::is_integral_v<RealOut>) {
        tf::parallel_copy(
            tf::make_points(tf::make_indirect_range(
