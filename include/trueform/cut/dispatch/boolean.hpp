...
auto tag_with_structures(const tf::polygons<Policy> &p, std::tuple<Ts...> &t) {
  return std::apply(
      [&p](auto &...structs) { return (p | ... | tf::tag(structs)); }, t);
}
...
auto boolean(const tf::polygons<Policy0> &_polygons0,
             const tf::polygons<Policy1> &_polygons1, F &&f) {
  using S0 = decltype(make_missing_structures(_polygons0));
  using S1 = decltype(make_missing_structures(_polygons1));
  ...
  if constexpr (std::is_same_v<S0, tf::none_t> &&
    ...
    S1 s1;
    tbb::parallel_invoke(
        [&] { s0 = make_missing_structures(_polygons0); },
        [&] { s1 = make_missing_structures(_polygons1); });
    return f(tag_with_structures(_polygons0, s0),
             tag_with_structures(_polygons1, s1));
  } else if constexpr (!std::is_same_v<S0, tf::none_t>) {
    auto s0 = make_missing_structures(_polygons0);
    return f(tag_with_structures(_polygons0, s0), _polygons1);
  } else {
    auto s1 = make_missing_structures(_polygons1);
