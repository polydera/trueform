...
#define TF_CPP_INSTANTIATE_MESH_BOUNDARY(Ngon)                                 \
  template auto boundary_edges<std::int32_t, double, 2, Ngon>(                 \
      const mesh<std::int32_t, double, 2, Ngon> &) -> nd_array<std::int32_t>;  \
  template auto boundary_paths<std::int32_t, double, 2, Ngon>(                 \
      const mesh<std::int32_t, double, 2, Ngon> &)                             \
      -> offset_blocked_buffer<std::int32_t, std::int32_t>;                    \
  template auto boundary_curves<std::int32_t, double, 2, Ngon>(                \
      const mesh<std::int32_t, double, 2, Ngon> &)                             \
      -> boundary_curves_result<std::int32_t, double, 2>;                      \
  template auto boundary_rims<std::int32_t, double, 2, Ngon>(                  \
      const mesh<std::int32_t, double, 2, Ngon> &)                             \
