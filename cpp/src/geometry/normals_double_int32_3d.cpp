...
#define TF_CPP_INSTANTIATE_MESH_NORMALS(Ngon)                                  \
  template auto normals<std::int32_t, double, 3, Ngon>(                        \
      const mesh<std::int32_t, double, 3, Ngon> &) -> nd_array<double>;        \
  template auto point_normals<std::int32_t, double, 3, Ngon>(                  \
      const mesh<std::int32_t, double, 3, Ngon> &) -> nd_array<double>
