...
namespace tf::cpp {
TF_CPP_INSTANTIATE_TYPED_REINDEX(float, std::int64_t, 2);

#define TF_CPP_INSTANTIATE_REINDEX_AT_ARITY(Ngon)                              \
  TF_CPP_INSTANTIATE_MESH_REINDEX(float, std::int64_t, 2, Ngon)
...
TF_CPP_INSTANTIATE_TRIANGLE_SOUP_REINDEX(float, std::int64_t, 2);
#if TF_CPP_MATRIX_HAS_DYNAMIC
TF_CPP_INSTANTIATE_MIXED_SOUP_REINDEX(float, std::int64_t, 2);
#endif
