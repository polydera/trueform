...
namespace tf::cpp {
TF_CPP_INSTANTIATE_TYPED_REINDEX(float, std::int32_t, 3);

#define TF_CPP_INSTANTIATE_REINDEX_AT_ARITY(Ngon)                              \
  TF_CPP_INSTANTIATE_MESH_REINDEX(float, std::int32_t, 3, Ngon)
...
#define TF_CPP_INSTANTIATE_REINDEX_PAIR(Index1, Real1, Ngon0, Ngon1)           \
  TF_CPP_INSTANTIATE_HETEROGENEOUS_PAIR_AT(float, std::int32_t, Real1, Index1, \
                                           3, Ngon0, Ngon1)

#define TF_CPP_INSTANTIATE_REINDEX_EDGE_PAIR(Index1, Real1)                    \
  TF_CPP_INSTANTIATE_HETEROGENEOUS_EDGE_PAIR(float, std::int32_t, Real1,       \
                                             Index1, 3)
...
TF_CPP_INSTANTIATE_TRIANGLE_SOUP_REINDEX(float, std::int32_t, 3);
#if TF_CPP_MATRIX_HAS_DYNAMIC
TF_CPP_INSTANTIATE_MIXED_SOUP_REINDEX(float, std::int32_t, 3);
#endif
