...
#define TF_CPP_INSTANTIATE_EDGE_MESH_RAY_CAST(RayReal)                         \
  template auto ray_cast<RayReal, TF_CPP_RAY_CAST_INDEX,                       \
                         TF_CPP_RAY_CAST_FORM_REAL, TF_CPP_RAY_CAST_DIMS>(     \
      const primitive<RayReal, TF_CPP_RAY_CAST_DIMS> &,                        \
      const edge_mesh<TF_CPP_RAY_CAST_INDEX, TF_CPP_RAY_CAST_FORM_REAL,        \
                      TF_CPP_RAY_CAST_DIMS> &,                                 \
      const ray_cast_options<TF_CPP_RAY_CAST_FORM_REAL> &)                     \
      -> ray_cast_form_result<TF_CPP_RAY_CAST_INDEX,                           \
                              TF_CPP_RAY_CAST_FORM_REAL>

TF_CPP_MATRIX_FOR_EACH_REAL(TF_CPP_INSTANTIATE_EDGE_MESH_RAY_CAST)

#undef TF_CPP_INSTANTIATE_EDGE_MESH_RAY_CAST
