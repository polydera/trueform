...
auto fit_knn(Resolver &&resolver, const point_cloud<Real, Dims> &source,
             const point_cloud<Real, Dims> &target,
             const fit_knn_options<Real> &options = {}) {
  auto owned_options = options;
  return submit<nd_array<Real>>(
      std::forward<Resolver>(resolver),
      [source, target, options = std::move(owned_options)] {
        return cpp::fit_knn(source, target, options);
      });
}
...
auto fit_knn(const point_cloud<Real, Dims> &source,
  ...
  return async::fit_knn(future_resolver{}, source, target, options);
}
