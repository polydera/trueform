...
auto symmetric_chamfer_error(Resolver &&resolver,
                             const point_cloud<Real, Dims> &source,
  ...
  return submit<Real>(std::forward<Resolver>(resolver),
                      [source, target, options = std::move(owned_options)] {
                        return cpp::symmetric_chamfer_error(source, target,
                                                            options);
                      });
}
...
auto symmetric_chamfer_error(const point_cloud<Real, Dims> &source,
                             ...
                             const chamfer_error_options<Real> &options = {})
    -> std::future<Real> {
  return async::symmetric_chamfer_error(future_resolver{}, source, target,
                                        options);
}
