...
template <typename Resolver, typename Index, typename Real, std::size_t Dims,
          std::size_t Ngon>
auto max_edge_length(Resolver &&resolver,
                     const cpp::mesh<Index, Real, Dims, Ngon> &value) {
  return submit<Real>(std::forward<Resolver>(resolver),
                      [value] { return cpp::max_edge_length(value); });
}

template <typename Index, typename Real, std::size_t Dims, std::size_t Ngon>
auto max_edge_length(const cpp::mesh<Index, Real, Dims, Ngon> &value)
    -> std::future<Real> {
  return async::max_edge_length(future_resolver{}, value);
}
