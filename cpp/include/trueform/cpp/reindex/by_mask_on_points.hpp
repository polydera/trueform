...
/// @brief Select mesh points by mask, keeping the carrier's index width.
...
/// @brief Select typed edge-mesh points by mask.
template <typename Index, typename Real, std::size_t Dims>
auto reindexed_by_mask_on_points(
    const edge_mesh<Index, Real, Dims> &value,
    const nd_array<std::int8_t> &mask) -> tf::segments_buffer<Index, Real, Dims>;
...
template <typename Index, typename Real, std::size_t Dims>
auto reindexed_by_mask_on_points(const nd_array<Index> &faces,
...
auto reindexed_by_mask_on_points(const nd_array<Index> &elements,
                                 const nd_array<Real> &points,
                                 const nd_array<std::int8_t> &mask)
    -> reindexed_fixed_carrier_t<Index, Real, Dims, Vertices>;

#define TF_CPP_REINDEX_EXTERN_BY_MASK_ON_POINTS_MESH(Index, Real, Dims, Ngon)  \
  extern template auto reindexed_by_mask_on_points<Index, Real, Dims, Ngon>(   \
...
#define TF_CPP_REINDEX_EXTERN_BY_MASK_ON_POINTS(Index, Real, Dims)             \
  extern template auto reindexed_by_mask_on_points<Index, Real, Dims>(         \
      const edge_mesh<Index, Real, Dims> &,                             \
      const nd_array<std::int8_t> &) -> tf::segments_buffer<Index, Real, Dims>;   \
  extern template auto                                                         \
  reindexed_by_mask_on_points_with_maps<Index, Real, Dims>(                    \
      const edge_mesh<Index, Real, Dims> &,                             \
      const nd_array<std::int8_t> &)                                           \
      -> reindexed_edge_mesh_result<Index, Real, Dims>;                        \
  extern template auto reindexed_by_mask_on_points<Index, Real, Dims, 2>(      \
      const nd_array<Index> &, const nd_array<Real> &,                         \
      const nd_array<std::int8_t> &)                                           \
      -> reindexed_fixed_carrier_t<Index, Real, Dims, 2>;                      \
  extern template auto reindexed_by_mask_on_points<Index, Real, Dims, 3>(      \
      const nd_array<Index> &, const nd_array<Real> &,                         \
      const nd_array<std::int8_t> &)                                           \
      -> reindexed_fixed_carrier_t<Index, Real, Dims, 3>

TF_CPP_MATRIX_FOR_EACH_INDEX_REAL_DIMS_NGON(
    TF_CPP_REINDEX_EXTERN_BY_MASK_ON_POINTS_MESH)
TF_CPP_MATRIX_FOR_EACH_INDEX_REAL_DIMS(TF_CPP_REINDEX_EXTERN_BY_MASK_ON_POINTS)

#undef TF_CPP_REINDEX_EXTERN_BY_MASK_ON_POINTS
#undef TF_CPP_REINDEX_EXTERN_BY_MASK_ON_POINTS_MESH
