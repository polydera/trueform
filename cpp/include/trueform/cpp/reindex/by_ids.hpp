...
/// @brief Select raw [N, Dims] points by IDs in their encoded index dtype.
...
/// @brief Select mesh faces by IDs, keeping the carrier's index width.
...
/// @brief Select typed edge-mesh edges by IDs.
template <typename Index, typename Real, std::size_t Dims>
...
template <typename Index, typename Real, std::size_t Dims>
auto reindexed_by_ids(const nd_array<Index> &faces,
                      const nd_array<Real> &points, const nd_array<Index> &ids)
    -> tf::polygons_buffer<Index, Real, Dims, 3>;

template <typename Index, typename Real, std::size_t Dims>
auto reindexed_by_ids(const offset_blocked_buffer<Index, Index> &faces,
                      const nd_array<Real> &points, const nd_array<Index> &ids)
    -> tf::polygons_buffer<Index, Real, Dims, tf::dynamic_size>;

template <typename Index, typename Real, std::size_t Dims>
auto reindexed_by_ids_with_maps(const nd_array<Index> &faces,
...
auto reindexed_by_ids(const nd_array<Index> &elements,
                      const nd_array<Real> &points, const nd_array<Index> &ids)
    -> reindexed_fixed_carrier_t<Index, Real, Dims, Vertices>;

template <typename Index, typename Real, std::size_t Dims, std::size_t Vertices,
          std::enable_if_t<Vertices == 2 || Vertices == 3, int> = 0>
auto reindexed_by_ids_with_maps(const nd_array<Index> &elements,
                                const nd_array<Real> &points,
                                const nd_array<Index> &ids)
    -> reindexed_fixed_result_t<Index, Real, Dims, Vertices>;

#define TF_CPP_REINDEX_EXTERN_BY_IDS_MESH(Index, Real, Dims, Ngon)             \
  extern template auto reindexed_by_ids<Index, Real, Dims, Ngon>(              \
      const mesh<Index, Real, Dims, Ngon> &, const nd_array<Index> &)          \
      -> tf::polygons_buffer<Index, Real, Dims, Ngon>;                                 \
  extern template auto reindexed_by_ids_with_maps<Index, Real, Dims, Ngon>(    \
      const mesh<Index, Real, Dims, Ngon> &, const nd_array<Index> &)          \
      -> reindexed_mesh_result<Index, Real, Dims, Ngon>

#define TF_CPP_REINDEX_EXTERN_BY_IDS(Index, Real, Dims)                        \
  extern template auto reindexed_by_ids<Index, Real, Dims>(                    \
      const nd_array<Real> &, const nd_array<Index> &) -> nd_array<Real>;      \
  extern template auto reindexed_by_ids_with_maps<Index, Real, Dims>(          \
  ...
  extern template auto reindexed_by_ids_with_maps<Index, Real, Dims>(          \
      const edge_mesh<Index, Real, Dims> &, const nd_array<Index> &)    \
      -> reindexed_edge_mesh_result<Index, Real, Dims>;                        \
  extern template auto reindexed_by_ids<Index, Real, Dims, 2>(                 \
      const nd_array<Index> &, const nd_array<Real> &,                         \
      const nd_array<Index> &)                                                 \
      -> reindexed_fixed_carrier_t<Index, Real, Dims, 2>;                      \
  extern template auto reindexed_by_ids_with_maps<Index, Real, Dims, 2>(       \
      const nd_array<Index> &, const nd_array<Real> &,                         \
      const nd_array<Index> &)                                                 \
      -> reindexed_fixed_result_t<Index, Real, Dims, 2>;                       \
  extern template auto reindexed_by_ids<Index, Real, Dims, 3>(                 \
      const nd_array<Index> &, const nd_array<Real> &,                         \
      const nd_array<Index> &)                                                 \
      -> reindexed_fixed_carrier_t<Index, Real, Dims, 3>;                      \
  extern template auto reindexed_by_ids_with_maps<Index, Real, Dims, 3>(       \
      const nd_array<Index> &, const nd_array<Real> &,                         \
      const nd_array<Index> &)                                                 \
      -> reindexed_fixed_result_t<Index, Real, Dims, 3>

TF_CPP_MATRIX_FOR_EACH_INDEX_REAL_DIMS_NGON(TF_CPP_REINDEX_EXTERN_BY_IDS_MESH)
TF_CPP_MATRIX_FOR_EACH_INDEX_REAL_DIMS(TF_CPP_REINDEX_EXTERN_BY_IDS)

#undef TF_CPP_REINDEX_EXTERN_BY_IDS
#undef TF_CPP_REINDEX_EXTERN_BY_IDS_MESH
