...
/// @brief Select raw [N, Dims] points by an int8 boolean mask.
...
/// @brief Select mesh faces by mask, keeping the carrier's index width.
...
/// @brief Select typed edge-mesh edges by mask.
template <typename Index, typename Real, std::size_t Dims>
...
template <typename Index, typename Real, std::size_t Dims>
auto reindexed_by_mask(const nd_array<Index> &faces,
                       const nd_array<Real> &points,
                       const nd_array<std::int8_t> &mask)
    -> tf::polygons_buffer<Index, Real, Dims, 3>;

template <typename Index, typename Real, std::size_t Dims>
auto reindexed_by_mask(const offset_blocked_buffer<Index, Index> &faces,
                       const nd_array<Real> &points,
                       const nd_array<std::int8_t> &mask)
    -> tf::polygons_buffer<Index, Real, Dims, tf::dynamic_size>;

template <typename Index, typename Real, std::size_t Dims>
auto reindexed_by_mask_with_maps(const nd_array<Index> &faces,
                                 const nd_array<Real> &points,
                                 const nd_array<std::int8_t> &mask)
...
auto reindexed_by_mask_with_maps(
    ...
    const nd_array<Real> &points, const nd_array<std::int8_t> &mask)
...
auto reindexed_by_mask(const nd_array<Index> &elements,
                       const nd_array<Real> &points,
                       const nd_array<std::int8_t> &mask)
...
template <typename Index, typename Real, std::size_t Dims, std::size_t Vertices,
          std::enable_if_t<Vertices == 2 || Vertices == 3, int> = 0>
auto reindexed_by_mask_with_maps(const nd_array<Index> &elements,
                                 const nd_array<Real> &points,
                                 const nd_array<std::int8_t> &mask)
    -> reindexed_fixed_result_t<Index, Real, Dims, Vertices>;

#define TF_CPP_REINDEX_EXTERN_BY_MASK_MESH(Index, Real, Dims, Ngon)            \
  extern template auto reindexed_by_mask<Index, Real, Dims, Ngon>(             \
      const mesh<Index, Real, Dims, Ngon> &, const nd_array<std::int8_t> &)    \
      -> tf::polygons_buffer<Index, Real, Dims, Ngon>;                                 \
  extern template auto reindexed_by_mask_with_maps<Index, Real, Dims, Ngon>(   \
      const mesh<Index, Real, Dims, Ngon> &, const nd_array<std::int8_t> &)    \
      -> reindexed_mesh_result<Index, Real, Dims, Ngon>

#define TF_CPP_REINDEX_EXTERN_BY_MASK(Index, Real, Dims)                       \
  extern template auto reindexed_by_mask<Index, Real, Dims>(                   \
      const nd_array<Real> &, const nd_array<std::int8_t> &)                   \
      -> nd_array<Real>;                                                       \
  extern template auto reindexed_by_mask_with_maps<Index, Real, Dims>(         \
      const nd_array<Real> &, const nd_array<std::int8_t> &)                   \
      -> reindexed_points_result<Index, Real>;                                 \
  extern template auto reindexed_by_mask<Index, Real, Dims>(                   \
      const edge_mesh<Index, Real, Dims> &,                             \
      const nd_array<std::int8_t> &) -> tf::segments_buffer<Index, Real, Dims>;   \
  extern template auto reindexed_by_mask_with_maps<Index, Real, Dims>(         \
      const edge_mesh<Index, Real, Dims> &,                             \
      const nd_array<std::int8_t> &)                                           \
      -> reindexed_edge_mesh_result<Index, Real, Dims>;                        \
  extern template auto reindexed_by_mask<Index, Real, Dims, 2>(                \
      const nd_array<Index> &, const nd_array<Real> &,                         \
      const nd_array<std::int8_t> &)                                           \
      -> reindexed_fixed_carrier_t<Index, Real, Dims, 2>;                      \
  extern template auto reindexed_by_mask_with_maps<Index, Real, Dims, 2>(      \
      const nd_array<Index> &, const nd_array<Real> &,                         \
      const nd_array<std::int8_t> &)                                           \
      -> reindexed_fixed_result_t<Index, Real, Dims, 2>;                       \
  extern template auto reindexed_by_mask<Index, Real, Dims, 3>(                \
      const nd_array<Index> &, const nd_array<Real> &,                         \
      const nd_array<std::int8_t> &)                                           \
      -> reindexed_fixed_carrier_t<Index, Real, Dims, 3>;                      \
  extern template auto reindexed_by_mask_with_maps<Index, Real, Dims, 3>(      \
      const nd_array<Index> &, const nd_array<Real> &,                         \
      const nd_array<std::int8_t> &)                                           \
      -> reindexed_fixed_result_t<Index, Real, Dims, 3>

TF_CPP_MATRIX_FOR_EACH_INDEX_REAL_DIMS_NGON(TF_CPP_REINDEX_EXTERN_BY_MASK_MESH)
TF_CPP_MATRIX_FOR_EACH_INDEX_REAL_DIMS(TF_CPP_REINDEX_EXTERN_BY_MASK)

#undef TF_CPP_REINDEX_EXTERN_BY_MASK
#undef TF_CPP_REINDEX_EXTERN_BY_MASK_MESH
