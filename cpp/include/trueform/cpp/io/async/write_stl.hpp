...
template <typename Resolver, typename Index, typename Real, std::size_t Dims,
          std::size_t Ngon, std::enable_if_t<Dims == 3 && Ngon == 3, int> = 0>
auto write_stl(Resolver &&resolver,
  ...
  return submit<nd_array<std::int8_t>>(
      ...
      [value] { return cpp::write_stl(value); });
}

template <typename Index, typename Real, std::size_t Dims, std::size_t Ngon,
          std::enable_if_t<Dims == 3 && Ngon == 3, int> = 0>
auto write_stl(const cpp::mesh<Index, Real, Dims, Ngon> &value)
    -> std::future<nd_array<std::int8_t>> {
  return async::write_stl(future_resolver{}, value);
}
...
auto write_stl(Resolver &&resolver,
  ...
  return submit<bool>(std::forward<Resolver>(resolver),
                      [value, path = std::move(owned_path)] {
                        return cpp::write_stl(value, path);
                      });
}
...
auto write_stl(const cpp::mesh<Index, Real, Dims, Ngon> &value,
  ...
  return async::write_stl(future_resolver{}, value, path);
}
