/**
 * Polygons to polygons closest-point FCL - Implementation
 *
...
int run_polygons_to_polygons_closest_point_fcl_benchmark(
  ...
  for (const auto &path : mesh_paths) {
    ...
    /*    [&]() {*/
    /*      dres.clear();*/
    /*      Scalar dist = ::fcl::distance(&obj_fixed_aabb, &obj_moving_aabb, dreq, dres);*/
    ...
    auto time_obbrss = benchmark::mean_time_of(
        ...
        [&]() {
          dres.clear();
          Scalar dist = ::fcl::distance(&obj_fixed_obbrss, &obj_moving_obbrss, dreq, dres);
          benchmark::do_not_optimize(dist);
          benchmark::do_not_optimize(dres.min_distance);
        },
        n_samples);
