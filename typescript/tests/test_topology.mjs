...
describe("Topology", () => {
  ...
  test("isClosed / isOpen (sphere vs plane, float64)", () => {
    const tf = getTf();
    ...
    assert(plane.dtype === "float64", "plane dtype");

    assert(tf.isClosed(sphere) === true, "sphere should be closed");
    assert(tf.isOpen(sphere) === false, "sphere should not be open");
    log("  sphere(f64): closed=true, open=false", "line-pass");

    assert(tf.isClosed(plane) === false, "plane should not be closed");
    assert(tf.isOpen(plane) === true, "plane should be open");
    log("  plane(f64): closed=false, open=true", "line-pass");
  ...
  // ==========================================================================
  test("isManifold / isNonManifold (sphere, float64)", () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    assert(sphere.dtype === "float64", "sphere dtype");
    ...
    assert(tf.isManifold(sphere) === true, "sphere should be manifold");
    assert(tf.isNonManifold(sphere) === false, "sphere should not be non-manifold");
    log("  sphere(f64): manifold=true, nonManifold=false", "line-pass");

    sphere.delete();
  });

  // ==========================================================================
  test("eulerCharacteristic (sphere = 2, torus = 0, float64)", () => {
    const tf = getTf();

    const sphere = tf.sphereMesh(1.0, 16, 16, { dtype: "float64" });
    assert(sphere.dtype === "float64", "sphere dtype");
    const ec = tf.eulerCharacteristic(sphere);
    assert(ec === 2, `expected euler=2 for sphere, got ${ec}`);
    log(`  sphere(f64) euler = ${ec}`, "line-pass");

    const box = tf.boxMesh(4, 4, 4);
    const cyl = tf.cylinderMesh(0.5, 6, 32);
    const diff = tf.booleanDifference(box, cyl);
    const torus32 = diff.mesh;
    const torus = tf.mesh(torus32.faces.data, Float64Array.from(torus32.points.data));
    assert(torus.dtype === "float64", "torus dtype");
    const ecTorus = tf.eulerCharacteristic(torus);
    assert(ecTorus === 0, `expected euler=0 for torus, got ${ecTorus}`);
    log(`  torus(f64) euler = ${ecTorus}`, "line-pass");
  ...
  // ==========================================================================
  test("boundaryEdges (sphere = 0, plane > 0, float64)", () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const be = tf.boundaryEdges(sphere);
    assert(be.shape[0] === 0, `expected 0 boundary edges for sphere, got ${be.shape[0]}`);
    log("  sphere(f64): 0 boundary edges", "line-pass");
    be.delete();
    ...
    assert(bePlane.shape[0] === 4, `expected 4 boundary edges for plane, got ${bePlane.shape[0]}`);
    assert(bePlane.shape[1] === 2, `expected shape[1]=2, got ${bePlane.shape[1]}`);
    log(`  plane(f64): ${bePlane.shape[0]} boundary edges`, "line-pass");
    bePlane.delete();
  ...
  // ==========================================================================
  test("nonManifoldEdges (sphere = 0, float64)", () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const nme = tf.nonManifoldEdges(sphere);
    assert(nme.shape[0] === 0, `expected 0 non-manifold edges, got ${nme.shape[0]}`);
    log("  sphere(f64): 0 non-manifold edges", "line-pass");
    nme.delete();
    sphere.delete();
  });

  // ==========================================================================
  test("boundaryPaths (plane = 1 loop, float64)", () => {
    const tf = getTf();
    const plane = tf.planeMesh(10, 5, undefined, undefined, { dtype: "float64" });
    const bp = tf.boundaryPaths(plane);
    assert(bp.length === 1, `expected 1 boundary path for plane, got ${bp.length}`);
    ...
    const pathData = path.data;
    assert(pathData[0] === pathData[path.shape[0] - 1], "closed path: first == last vertex");
    log(`  plane(f64): 1 closed boundary path with ${path.shape[0]} vertices`, "line-pass");
    path.delete();
    bp.delete();
    plane.delete();
  });
  ...
  // ==========================================================================
  test("kRings (sphere, k=1, float64)", () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const kr = tf.kRings(sphere, 1);
    assert(kr.length === sphere.numberOfPoints, `expected ${sphere.numberOfPoints} rings, got ${kr.length}`);
    ...
    const ring0 = kr.get(0);
    assert(ring0.shape[0] >= 3, `expected >=3 neighbors, got ${ring0.shape[0]}`);
    log(`  vertex 0(f64) has ${ring0.shape[0]} 1-ring neighbors`, "line-pass");
  ...
  test("kRings inclusive (float64)", () => {
    ...
    assert(containsSeed, "inclusive k-ring should contain seed vertex");
    log("  inclusive k-ring(f64) contains seed vertex", "line-pass");
  ...
  // ==========================================================================
  test("neighborhoods (sphere, float64)", () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const nbr = tf.neighborhoods(sphere, 0.5);
    assert(nbr.length === sphere.numberOfPoints, `expected ${sphere.numberOfPoints} neighborhoods`);
    ...
    const n0 = nbr.get(0);
    assert(n0.shape[0] >= 1, `expected >=1 neighbors within radius, got ${n0.shape[0]}`);
    log(`  vertex 0(f64): ${n0.shape[0]} neighbors within r=0.5`, "line-pass");
  ...
  // ==========================================================================
  test("connectEdgesToPaths (float64)", () => {
    const tf = getTf();
    const edges = tf.ndarray(new Int32Array([0, 1, 1, 2, 2, 0]), [3, 2]);
    const paths = tf.connectEdgesToPaths(edges);
    assert(paths.length === 1, `expected 1 path, got ${paths.length}`);
  ...
  // ==========================================================================
  test("labelConnectedComponents (OffsetBlockedBuffer — vertexLink, float64)", () => {
    const tf = getTf();
    const s1 = tf.sphereMesh(1.0, 8, 8, { dtype: "float64" });
    const s2 = tf.sphereMesh(1.0, 8, 8, { dtype: "float64" });

    const p2 = s2.points;
    const p2data = p2.data;
    const offset = new Float64Array(p2data.length);
    for (let i = 0; i < p2data.length; i += 3) {
      offset[i] = p2data[i] + 5;
      offset[i + 1] = p2data[i + 1];
      offset[i + 2] = p2data[i + 2];
    ...
    const combinedPoints = new Float64Array(p2data.length + s1.numberOfPoints * 3);
    const p1 = s1.points;
    combinedPoints.set(p1.data);
    combinedPoints.set(offset, s1.numberOfPoints * 3);

    const combined = tf.mesh(combinedFaces, combinedPoints);
    assert(combined.dtype === "float64", "combined dtype");
    const vl = combined.vertexLink;
    const result = tf.labelConnectedComponents(vl);
    assert(result.nComponents === 2, `expected 2 components, got ${result.nComponents}`);
    assert(result.labels.shape[0] === combined.numberOfPoints,
      `expected ${combined.numberOfPoints} labels, got ${result.labels.shape[0]}`);
    log(`  2 spheres(f64) combined → ${result.nComponents} vertex components`, "line-pass");
  ...
  // ==========================================================================
  test("labelConnectedComponents (NDArrayInt32 — manifoldEdgeLink, float64)", () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const mel = sphere.manifoldEdgeLink;
    const result = tf.labelConnectedComponents(mel);
    ...
    assert(result.labels.shape[0] === sphere.numberOfFaces,
      `expected ${sphere.numberOfFaces} labels, got ${result.labels.shape[0]}`);
    log(`  sphere(f64) → ${result.nComponents} manifold-edge component`, "line-pass");
  ...
  // ==========================================================================
  test("connectedComponents convenience (edge, manifoldEdge, vertex, float64)", () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });

    const edge = tf.connectedComponents(sphere, "edge");
    assert(edge.nComponents === 1, `expected 1 edge component, got ${edge.nComponents}`);
    log(`  sphere(f64) edge components: ${edge.nComponents}`, "line-pass");

    const me = tf.connectedComponents(sphere, "manifoldEdge");
    assert(me.nComponents === 1, `expected 1 manifoldEdge component, got ${me.nComponents}`);
    log(`  sphere(f64) manifoldEdge components: ${me.nComponents}`, "line-pass");

    const vtx = tf.connectedComponents(sphere, "vertex");
    assert(vtx.nComponents === 1, `expected 1 vertex component, got ${vtx.nComponents}`);
    log(`  sphere(f64) vertex components: ${vtx.nComponents}`, "line-pass");
  ...
  // ==========================================================================
  test("consistentlyOriented (preserves face/point count, float64)", () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const oriented = tf.consistentlyOriented(sphere);
    assert(oriented.dtype === "float64", "oriented dtype");

    assert(oriented.numberOfFaces === sphere.numberOfFaces,
      `face count: expected ${sphere.numberOfFaces}, got ${oriented.numberOfFaces}`);
    assert(oriented.numberOfPoints === sphere.numberOfPoints,
      `point count: expected ${sphere.numberOfPoints}, got ${oriented.numberOfPoints}`);
    log(`  ${oriented.numberOfFaces} faces, ${oriented.numberOfPoints} points (preserved, f64)`, "line-pass");
  ...
  // ==========================================================================
  // ASYNC FLOAT64 MIRRORS
  // ==========================================================================

  test("async: isClosed / isOpen (float64)", async () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const plane = tf.planeMesh(10, 5, undefined, undefined, { dtype: "float64" });
    assert(await tf.async.isClosed(sphere) === true, "sphere closed");
    assert(await tf.async.isOpen(plane) === true, "plane open");
    log("  async isClosed/isOpen(f64) OK", "line-pass");
    plane.delete(); sphere.delete();
  });

  test("async: isManifold / isNonManifold (float64)", async () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    assert(await tf.async.isManifold(sphere) === true, "manifold");
    assert(await tf.async.isNonManifold(sphere) === false, "not non-manifold");
    log("  async isManifold/isNonManifold(f64) OK", "line-pass");
    sphere.delete();
  });

  test("async: eulerCharacteristic (float64)", async () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 16, 16, { dtype: "float64" });
    const ec = await tf.async.eulerCharacteristic(sphere);
    assert(ec === 2, `expected 2, got ${ec}`);
    log(`  async euler(f64) = ${ec}`, "line-pass");
    sphere.delete();
  });

  test("async: boundaryEdges (float64)", async () => {
    const tf = getTf();
    const plane = tf.planeMesh(10, 5, undefined, undefined, { dtype: "float64" });
    const be = await tf.async.boundaryEdges(plane);
    assert(be.shape[0] === 4, `expected 4 boundary edges, got ${be.shape[0]}`);
    log(`  async boundaryEdges(f64): ${be.shape[0]}`, "line-pass");
    be.delete(); plane.delete();
  });

  test("async: nonManifoldEdges (float64)", async () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const nme = await tf.async.nonManifoldEdges(sphere);
    assert(nme.shape[0] === 0, `expected 0, got ${nme.shape[0]}`);
    log("  async nonManifoldEdges(f64): 0", "line-pass");
    nme.delete(); sphere.delete();
  });

  test("async: boundaryPaths (float64)", async () => {
    const tf = getTf();
    const plane = tf.planeMesh(10, 5, undefined, undefined, { dtype: "float64" });
    const bp = await tf.async.boundaryPaths(plane);
    assert(bp.length === 1, `expected 1 path, got ${bp.length}`);
    log(`  async boundaryPaths(f64): ${bp.length} path`, "line-pass");
    bp.delete(); plane.delete();
  });

  test("async: kRings (float64)", async () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const kr = await tf.async.kRings(sphere, 1);
    assert(kr.length === sphere.numberOfPoints, "ring count matches");
    log(`  async kRings(f64): ${kr.length} rings`, "line-pass");
    kr.delete(); sphere.delete();
  });

  test("async: neighborhoods (float64)", async () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const nbr = await tf.async.neighborhoods(sphere, 0.5);
    assert(nbr.length === sphere.numberOfPoints, "neighborhood count matches");
    log(`  async neighborhoods(f64): ${nbr.length}`, "line-pass");
    nbr.delete(); sphere.delete();
  });

  test("async: connectEdgesToPaths (float64)", async () => {
    const tf = getTf();
    const edges = tf.ndarray(new Int32Array([0, 1, 1, 2, 2, 0]), [3, 2]);
    const paths = await tf.async.connectEdgesToPaths(edges);
    assert(paths.length === 1, `expected 1 path, got ${paths.length}`);
    log(`  async connectEdgesToPaths(f64): ${paths.length} path`, "line-pass");
    paths.delete(); edges.delete();
  });

  test("async: connectedComponents (float64)", async () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const cc = await tf.async.connectedComponents(sphere, "edge");
    assert(cc.nComponents === 1, `expected 1, got ${cc.nComponents}`);
    log(`  async connectedComponents(f64): ${cc.nComponents}`, "line-pass");
    cc.labels.delete(); sphere.delete();
  });

  test("async: labelConnectedComponents (manifoldEdgeLink, float64)", async () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const mel = sphere.manifoldEdgeLink;
    const result = await tf.async.labelConnectedComponents(mel);
    assert(result.nComponents === 1, `expected 1 component, got ${result.nComponents}`);
    assert(result.labels.shape[0] === sphere.numberOfFaces, "labels match face count");
    log(`  async labelConnectedComponents(f64): ${result.nComponents}`, "line-pass");
    result.labels.delete(); mel.delete(); sphere.delete();
  });

  test("async: consistentlyOriented (float64)", async () => {
    const tf = getTf();
    const sphere = tf.sphereMesh(1.0, 10, 10, { dtype: "float64" });
    const oriented = await tf.async.consistentlyOriented(sphere);
    assert(oriented.dtype === "float64", "oriented dtype");
    assert(oriented.numberOfFaces === sphere.numberOfFaces, "face count preserved");
    log(`  async consistentlyOriented(f64): ${oriented.numberOfFaces} faces`, "line-pass");
    oriented.delete(); sphere.delete();
  });
