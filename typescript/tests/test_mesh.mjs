...
describe("Mesh", () => {
  ...
  test("Create mesh and read back (float64)", () => {
    const tf = getTf();
    const { faces, points } = twoTrianglesFloat64();
    const m = tf.mesh(faces, points);

    assert(m.dtype === "float64", `expected dtype float64, got ${m.dtype}`);
    log(`  m.dtype = ${m.dtype}`, "line-pass");
    ...
    const ph = m.points;
    assert(ph.dtype === "float64", `expected points dtype float64, got ${ph.dtype}`);
    const pd = ph.data;
    assert(pd instanceof Float64Array, "points data should be Float64Array");
    assert(pd.length === 12, `expected 12 point coords, got ${pd.length}`);
    log("  points data matches input (Float64Array)", "line-pass");
  ...
  test("Independent handle ownership (float64)", () => {
    ...
    assert(fh.data.length === 6, "faces data still accessible");
    assert(ph.data.length === 12, "points data still accessible");
    assert(ph.data instanceof Float64Array, "points data is Float64Array after mesh delete");
    log("  handles survive mesh.delete() (float64)", "line-pass");

    fh.delete();
    ph.delete();
  });

  test("Shallow copy (float64)", () => {
    const tf = getTf();
    const { faces, points } = twoTrianglesFloat64();
    const m = tf.mesh(faces, points);
    const v = m.shallowCopy();

    assert(v.dtype === "float64", "shallow copy preserves dtype");
    assert(v.numberOfFaces === 2, "shallow copy has same face count");
    assert(v.numberOfPoints === 4, "shallow copy has same point count");
  ...
  test("Face membership (float64)", () => {
    ...
    assert(totalMemberships === 6, `expected 6 total memberships, got ${totalMemberships}`);
    log(`  total memberships = ${totalMemberships} (float64)`, "line-pass");

    fm.delete();
    m.delete();
  });

  test("Manifold edge link (float64)", () => {
    const tf = getTf();
    const { faces, points } = tetrahedronFloat64();
    const m = tf.mesh(faces, points);

    assert(m.dtype === "float64", "tetrahedron mesh dtype is float64");
    ...
    assert(!hasBoundary, "tetrahedron should have no boundary edges");
    log("  no boundary edges on closed tetrahedron (float64)", "line-pass");

    mel.delete();
    m.delete();
  });

  test("Face link (float64)", () => {
    const tf = getTf();
    const { faces, points } = twoTrianglesFloat64();
    const m = tf.mesh(faces, points);
    const fl = m.faceLink;
    ...
    assert(found1, "face 0 should be adjacent to face 1");
    log("  face 0 adjacent to face 1 (float64)", "line-pass");

    fl.delete();
    m.delete();
  });

  test("Vertex link (float64)", () => {
    const tf = getTf();
    const { faces, points } = twoTrianglesFloat64();
    const m = tf.mesh(faces, points);
    const vl = m.vertexLink;
    ...
    const block0 = vl.get(0);
    assert(block0.length === 3, `vertex 0 should have 3 neighbors, got ${block0.length}`);
    log(`  vertex 0 neighbors: [${Array.from(block0)}] (float64)`, "line-pass");

    vl.delete();
    m.delete();
  });

  test("Generation tracking (float64, faces setter invalidates cache)", () => {
    const tf = getTf();
    const { faces, points } = twoTrianglesFloat64();
    const m = tf.mesh(faces, points);

    assert(m.dtype === "float64", "mesh is float64");
    ...
    assert(totalMemberships === 3, `expected 3 memberships after faces setter, got ${totalMemberships}`);
    log("  topology rebuilt after faces setter (float64)", "line-pass");
