...
EMSCRIPTEN_BINDINGS(trueform_reindex_split_into_domains_float64) {
  using Real = double;
  using namespace tf::ts;

  emscripten::value_object<split_domains_result_t<Real>>(
      "SplitDomainsResultFloat64")
      .field("components", &split_domains_result_t<Real>::components)
      .field("labels", &split_domains_result_t<Real>::labels);

  emscripten::function("split_into_domains_float64",
                       &sync_split_into_domains<Real>);
  emscripten::function("dispatch_split_into_domains_float64",
