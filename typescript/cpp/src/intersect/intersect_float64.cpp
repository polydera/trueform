...
EMSCRIPTEN_BINDINGS(trueform_intersect_float64) {
  using Real = double;
  using namespace tf::ts;

  // Intersection curves — sync/async
  emscripten::function("intersection_curves_float64",
                       &sync_intersection_curves<Real>);
  emscripten::function("dispatch_intersection_curves_float64",
                       &async_intersection_curves<Real>);

  // Self-intersection curves — sync/async
  emscripten::function("self_intersection_curves_float64",
  ...
  emscripten::function("dispatch_intersection_curves_list_float64",
