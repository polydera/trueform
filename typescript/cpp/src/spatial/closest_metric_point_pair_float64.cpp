...
EMSCRIPTEN_BINDINGS(trueform_spatial_closest_metric_point_pair_float64) {
  using Real = double;
  using namespace tf::ts;

  emscripten::value_object<metric_point_pair_result_t<Real>>(
      "MetricPointPairResultFloat64")
  ...
  emscripten::value_object<metric_point_pair_batch_result_t<Real>>(
      "MetricPointPairBatchResultFloat64")
  ...
  emscripten::function("closest_metric_point_pair_float64",
                       &sync_closest_metric_point_pair<Real>);
  emscripten::function("dispatch_closest_metric_point_pair_float64",
