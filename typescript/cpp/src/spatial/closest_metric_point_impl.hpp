...
template <typename Real> struct metric_point_result_t {
  wasm_ndarray<Real> point; // [3]
  Real distance2;
};

template <typename Real> struct metric_point_batch_result_t {
...
inline auto cmp_point_to_ndarray(const PointLike &pt) -> wasm_ndarray<Real> {
  tf::buffer<Real> buf;
  buf.allocate(3);
  auto *p = buf.data();
  p[0] = pt[0];
...
inline auto pp_single(const Real *a, prim_type ta, const Real *b, prim_type tb,
                      int va, int vb) -> metric_point_result_t<Real> {
  return dispatch_pair(
      [](const auto &pa, const auto &pb) -> metric_point_result_t<Real> {
        auto r = tf::closest_metric_point(pa, pb);
        return {cmp_point_to_ndarray<Real>(r.point),
...
inline auto pp_batch(wasm_ndarray<Real> &a, prim_type ta, wasm_ndarray<Real> &b,
                     prim_type tb) -> metric_point_batch_result_t<Real> {
  ...
  auto compute = [=](int i) {
    auto r = dispatch_pair(
        [](const auto &a, const auto &b) {
          return tf::closest_metric_point(a, b);
        },
  ...
  return {wasm_ndarray<Real>::from_buffer(std::move(pts), {n, 3}),
...
template <typename Real>
inline auto sync_closest_metric_point(wasm_ndarray<Real> &a, int ta_int,
...
template <typename Real>
inline auto async_closest_metric_point(wasm_ndarray<Real> &a, int ta_int,
                                       wasm_ndarray<Real> &b, int tb_int)
    -> promise_t {
  auto ta = static_cast<prim_type>(ta_int);
  auto tb = static_cast<prim_type>(tb_int);
  ...
  if (!is_batch(a, ta) && !is_batch(b, tb)) {
    ...
    int vb = poly_verts(b, tb);
    return promise(
        [a = a, b = b, ta, tb, va, vb]() -> metric_point_result_t<Real> {
  ...
  return promise(
      [a = a, b = b, ta, tb]() -> metric_point_batch_result_t<Real> {
