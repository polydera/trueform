...
EMSCRIPTEN_BINDINGS(trueform_cut_embedded_float32) {
  using Real = float;
  using namespace tf::ts;

  // Result types
  emscripten::value_object<cut_result_t<Real>>("CutResultFloat32")
  ...
  emscripten::function("embedded_intersection_curves_float32",
                       &sync_embedded_intersection_curves<Real>);
  emscripten::function("embedded_intersection_curves_with_curves_float32",
                       &sync_embedded_intersection_curves_with_curves<Real>);

  // Embedded intersection curves — async
  emscripten::function("dispatch_embedded_intersection_curves_float32",
                       &async_embedded_intersection_curves<Real>);
  emscripten::function(
      "dispatch_embedded_intersection_curves_with_curves_float32",
      &async_embedded_intersection_curves_with_curves<Real>);

  // Embedded self-intersection curves — sync
  emscripten::function("embedded_self_intersection_curves_float32",
                       &sync_embedded_self_intersection_curves<Real>);
  emscripten::function(
      "embedded_self_intersection_curves_with_curves_float32",
      &sync_embedded_self_intersection_curves_with_curves<Real>);

  // Embedded self-intersection curves — async
  emscripten::function("dispatch_embedded_self_intersection_curves_float32",
                       &async_embedded_self_intersection_curves<Real>);
  emscripten::function(
      "dispatch_embedded_self_intersection_curves_with_curves_float32",
